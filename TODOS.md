...
**Context:** Google shipped Chrome DevTools MCP in Chrome 146+ (June 2025). It provides screenshots, console messages, performance traces, Lighthouse audits, and full page interaction through the user's real browser. adeel should use it for real-session access while keeping Playwright for headless CI/testing workflows.
...
May replace `/adeel:setup-browser-cookies` for most use cases since the user's real cookies are already there.
...
`$B state save/load` ships in v0.12.1.0. V1 saves cookies + URLs only (not localStorage, which breaks on load-before-navigate). Files at `.adeel/browse-states/{name}.json` with 0o600 permissions. Load replaces session (closes all pages first). Name sanitized to `[a-zA-Z0-9_-]`.
...
`$B connect` launches Playwright's bundled Chromium in headed mode with the adeel Chrome extension auto-loaded. `$B handoff` now produces the same result (extension + side panel). Sidebar chat gated behind `--chat` flag.
...
**Why:** Enables parallel /adeel:qa + /adeel:design-review on different tabs in the same browser.
...
**What:** Publish the adeel browse Chrome extension to Chrome Web Store for easier install.
...
### GitLab support for /adeel:land-and-deploy

**What:** Add GitLab MR merge + CI polling support to `/adeel:land-and-deploy` skill. Currently uses `gh pr view`, `gh pr checks`, `gh pr merge`, and `gh run list/view` in 15+ places — each needs a GitLab conditional path using `glab ci status`, `glab mr merge`, etc.

**Why:** Without this, GitLab users can `/adeel:ship` (create MR) but can't `/adeel:land-and-deploy` (merge + verify). Completes the GitLab story end-to-end.

**Context:** `/adeel:retro`, `/adeel:ship`, and `/adeel:document-release` now support GitLab via the multi-platform `BASE_BRANCH_DETECT` resolver. `/adeel:land-and-deploy` has deeper GitHub-specific semantics (merge queues, required checks via `gh pr checks`, deploy workflow polling) that have different shapes on GitLab. The `glab` CLI (v1.90.0) supports `glab mr merge`, `glab ci status`, `glab ci view` but with different output formats and no merge queue concept.
...
**What:** Add a periodic E2E eval that creates a branch with 5+ commits spanning 3+ themes (features, cleanup, infra), runs /adeel:ship's Step 5 CHANGELOG generation, and verifies the CHANGELOG mentions all themes.

**Why:** The bug fixed in v0.11.22 (user/ship-full-commit-coverage) showed that /adeel:ship's CHANGELOG generation biased toward recent commits on long branches. The prompt fix adds a cross-check, but no test exercises the multi-commit failure mode. The existing `ship-local-workflow` E2E only uses a single-commit branch.
...
### Ship log — persistent record of /adeel:ship runs

**What:** Append structured JSON entry to `.adeel/ship-log.json` at end of every /adeel:ship run (version, date, branch, PR URL, review findings, Greptile stats, todos completed, test results).

**Why:** /adeel:retro has no structured data about shipping velocity. Ship log enables: PRs-per-week trending, review finding rates, Greptile signal over time, test suite growth.

**Context:** /adeel:retro already reads greptile-history.md — same pattern. Eval persistence (eval-store.ts) shows the JSON append pattern exists in the codebase. ~15 lines in ship template.
...
**What:** /adeel:ship Step 7.5: screenshot key pages after push, embed in PR body.
...
**Effort:** M
**Priority:** P2
**Depends on:** /setup-adeel-upload

## Review

### Inline PR annotations

**What:** /adeel:ship and /adeel:review post inline review comments at specific file:line locations using `gh api` to create pull request review comments.
...
**What:** /adeel:review Step 4.5: browse PR's preview deploy, annotated screenshots of changed pages, compare against production, check responsive layouts, verify accessibility tree.
...
**What:** `/adeel:qa` as GitHub Action step, fail PR if health score drops.
...
### /setup-adeel-upload skill (S3 bucket)

**What:** Configure S3 bucket for image hosting. One-time setup for visual PR annotations.

**Why:** Prerequisite for visual PR annotations in /adeel:ship and /adeel:review.
...
**What:** `browse/bin/adeel-upload` — upload file to S3, return public URL.
...
**Context:** Reads `$HOME/.adeel-dev/evals/*.json`. ~200 lines HTML + chart.js via Bun HTTP server.
...
**What:** Run `/adeel:qa` as a GitHub Action step, fail PR if health score drops below threshold.
...
**Context:** Requires headless browse binary available in CI. The `/adeel:qa` skill already produces `baseline.json` with health scores — CI step would compare against the main branch baseline and fail if score drops. Would need `ANTHROPIC_API_KEY` in CI secrets since `/adeel:qa` uses Claude.
...
**What:** `adeel-open-url` helper script — detect platform, use `open` (macOS) or `xdg-open` (Linux).

**Why:** The first-time Completeness Principle intro uses macOS `open` to launch the essay. If adeel ever supports Linux, this silently fails.
...
**Why:** Cross-team design discovery at scale. Local `$HOME/.adeel/projects/$SLUG/` keyword-grep discovery works for same-machine users now, but Supabase sync makes it work across the whole team. Duplicate ideas surface, everyone sees what's been explored.

**Context:** /adeel:office-hours writes design docs to `$HOME/.adeel/projects/$SLUG/`. The team store already syncs test plans, retro snapshots, QA reports. Design docs follow the same pattern — just add a sync adapter.

**Effort:** S
**Priority:** P2
**Depends on:** `user/team-supabase-store` branch landing on main

### /yc-prep skill

**What:** Skill that helps founders prepare their YC application after /adeel:office-hours identifies strong signal. Pulls from the design doc, structures answers to YC app questions, runs a mock interview.

**Why:** Closes the loop. /adeel:office-hours identifies the founder, /yc-prep helps them apply well. The design doc already contains most of the raw material for a YC application.
...
### /adeel:plan-design-review + /qa-design-review + /adeel:design-consultation — SHIPPED

Shipped as v0.5.0 on main. Includes `/adeel:plan-design-review` (report-only design audit), `/qa-design-review` (audit + fix loop), and `/adeel:design-consultation` (interactive DESIGN.md creation). `{{DESIGN_METHODOLOGY}}` resolver provides shared 80-item design audit checklist.

### Design outside voices in /adeel:plan-eng-review

**What:** Extend the parallel dual-voice pattern (Codex + Claude subagent) to /adeel:plan-eng-review's architecture review section.
...
### Outside voices in /adeel:qa visual regression detection

**What:** Add Codex design voice to /adeel:qa for detecting visual regressions during bug-fix verification.
...
**Context:** Depends on /adeel:qa having design awareness. Currently /adeel:qa focuses on functional testing.
...
### Auto-invoke /adeel:document-release from /adeel:ship — SHIPPED

Shipped in v0.8.3. Step 8.5 added to `/adeel:ship` — after creating the PR, `/adeel:ship` automatically reads `document-release/SKILL.md` and executes the doc update workflow. Zero-friction doc updates.

### `{{DOC_VOICE}}` shared resolver

**What:** Create a placeholder resolver in gen-skill-docs.ts encoding the adeel voice guide (friendly, user-forward, lead with benefits). Inject into /adeel:ship Step 5, /adeel:document-release Step 5, and reference from CLAUDE.md.

**Why:** DRY — voice rules currently live inline in 3 places (CLAUDE.md CHANGELOG style section, /adeel:ship Step 5, /adeel:document-release Step 5). When the voice evolves, all three drift.
...
`bin/adeel-diff-scope` shipped — categorizes diff into SCOPE_FRONTEND, SCOPE_BACKEND, SCOPE_PROMPTS, SCOPE_TESTS, SCOPE_DOCS, SCOPE_CONFIG. Used by design-review-lite to skip when no frontend files changed. Dashboard integration for conditional row display is a follow-up.
...
**Effort:** S
**Priority:** P3
**Depends on:** adeel-diff-scope (shipped)
...
**What:** A Codex-native skill (`.agents/skills/adeel-claude/SKILL.md`) that runs `claude -p` to get an independent second opinion from Claude — the reverse of what `/adeel:codex` does today from Claude Code.

**Why:** Codex users deserve the same cross-model challenge that Claude users get via `/adeel:codex`. Currently the flow is one-way (Claude→Codex). Codex users have no way to get a Claude second opinion.

**Context:** The `/adeel:codex` skill template (`codex/SKILL.md.tmpl`) shows the pattern — it wraps `codex exec` with JSONL parsing, timeout handling, and structured output. The reverse skill would wrap `claude -p` with similar infrastructure. Would be generated into `.agents/skills/adeel-claude/` by `gen-skill-docs --host codex`.
...
**What:** Track how often Claude chooses the complete option vs shortcut across adeel sessions. Aggregate into a dashboard showing completeness trend over time.
...
**Effort:** M (human) / S (CC)
**Priority:** P3
**Depends on:** Go all the way shipped (v0.6.1)

## Safety & Observability

### On-demand hook skills (/adeel:careful, /adeel:freeze, /adeel:guard) — SHIPPED

~~**What:** Three new skills that use Claude Code's session-scoped PreToolUse hooks to add safety guardrails on demand.~~

Shipped as `/adeel:careful`, `/adeel:freeze`, `/adeel:guard`, and `/adeel:unfreeze` in v0.6.5. Includes hook fire-rate telemetry (pattern name only, no command content) and inline skill activation telemetry.

### Skill usage telemetry — SHIPPED

~~**What:** Track which skills get invoked, how often, from which repo.~~

Shipped in v0.6.5. TemplateContext in gen-skill-docs.ts bakes skill name into preamble telemetry line. Analytics CLI (`bun run analytics`) for querying. /adeel:retro integration shows skills-used-this-week.

### /adeel:investigate scoped debugging enhancements (gated on telemetry)

**What:** Six enhancements to /adeel:investigate auto-freeze, contingent on telemetry showing the freeze hook actually fires in real debugging sessions.

**Why:** /adeel:investigate v0.7.1 auto-freezes edits to the module being debugged. If telemetry shows the hook fires often, these enhancements make the experience smarter. If it never fires, the problem wasn't real and these aren't worth building.
...
5. Debug session persistence ($HOME/.adeel/investigate-sessions/ — save investigation for reuse)
6. Investigation timeline in debug report (hypothesis log with timing)

**Effort:** M (all 6 combined)
**Priority:** P3
**Depends on:** Telemetry data showing freeze hook fires in real /adeel:investigate sessions
...
- /adeel:land-and-deploy — merge PR, wait for CI/deploy, canary verification
- /adeel:canary — post-deploy monitoring loop with anomaly detection
- /adeel:benchmark — performance regression detection with Core Web Vitals
- /adeel:setup-deploy — one-time deploy platform configuration
- /adeel:review Performance & Bundle Impact pass
...
- /adeel:qa SKILL.md with 6-phase workflow, 3 modes (full/quick/regression)
...
- Cookie picker web UI, /adeel:setup-browser-cookies skill
...
- Config CLI (`bin/adeel-config`), auto-upgrade via `$HOME/.adeel/config.yaml`, 12h cache TTL, exponential snooze backoff (24h→48h→1wk), "never ask again" option, vendored copy sync on upgrade
**Completed:** v0.3.8
