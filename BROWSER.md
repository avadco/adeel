...
This document covers the command reference and internals of adeel's headless browser.
...
adeel's browser is a compiled CLI binary that talks to a persistent local Chromium daemon over HTTP. The CLI is a thin client — it reads a state file, sends a command, and prints the response to stdout. The server does the real work via [Playwright](https://playwright.dev/).
...
Each server session generates a random UUID as a bearer token. The token is written to the state file (`.adeel/browse.json`) with chmod 600. Every HTTP request must include `Authorization: Bearer <token>`. This prevents other processes on the machine from controlling the browser.
...
- Console: `.adeel/browse-console.log`
- Network: `.adeel/browse-network.log`
- Dialog: `.adeel/browse-dialog.log`
...
The window has a subtle green shimmer line at the top edge and a floating "adeel" pill in the bottom-right corner so you always know which Chrome window is being controlled.
...
**CDP-aware skills:** When in real-browser mode, `/adeel:qa` and `/adeel:design-review` automatically skip cookie import prompts and headless workarounds.
...
# Click the adeel icon in toolbar → Open Side Panel
```
...
```bash
bin/adeel-extension    # opens chrome://extensions, copies path to clipboard
```
...
4. **Navigate to the extension folder:** Press **Cmd+Shift+G** in the file picker to open "Go to folder", then paste one of these paths:
   - Global install: `${CLAUDE_PLUGIN_ROOT}/extension`
   - Dev/source: `<adeel-repo>/extension`
...
5. **Pin it:** Click the puzzle piece icon (Extensions) in the toolbar → pin "adeel browse"
6. **Set the port:** Click the adeel icon → enter the port from `$B status` or `.adeel/browse.json`
7. **Open Side Panel:** Click the adeel icon → "Open Side Panel"
...
| **Connection pill** | Small "adeel" pill in the bottom-right corner of every page when connected |
...
2. Import cookies from your real Chrome via `/adeel:setup-browser-cookies`
...
Each workspace gets its own isolated browser instance with its own Chromium process, tabs, cookies, and logs. State is stored in `.adeel/` inside the project root (detected via `git rev-parse --show-toplevel`).

| Workspace | State file | Port |
|-----------|------------|------|
| `/code/project-a` | `/code/project-a/.adeel/browse.json` | random (10000-60000) |
| `/code/project-b` | `/code/project-b/.adeel/browse.json` | random (10000-60000) |
...
| `BROWSE_STATE_FILE` | `.adeel/browse.json` | Path to state file (CLI passes to server) |
...
| **adeel browse** | **~3s** | **~100-200ms** | **0 tokens** (plain text stdout) |

The context overhead difference compounds fast. In a 20-command browser session, MCP tools burn 30,000-40,000 tokens on protocol framing alone. adeel burns zero.
...
adeel skips all of this. Compiled binary. Plain text in, plain text out. No protocol. No schema. No connection management.
...
| File | Role |
|------|------|
| `browse/src/cli.ts` | Entry point. Reads `.adeel/browse.json`, sends HTTP to the server, prints response. |
...
The active skill lives at `${CLAUDE_PLUGIN_ROOT}/`. After making changes:

1. Push your branch
2. Pull in the skill directory: `cd ~/.claude/skills/adeel && git pull`
3. Rebuild: `cd ~/.claude/skills/adeel && bun run build`

Or copy the binary directly: `cp browse/dist/browse ${CLAUDE_PLUGIN_ROOT}/browse/dist/browse`
