...
bun run eval:list    # list all eval runs from $HOME/.adeel-dev/evals/
...
--verbose`). Results are persisted to `$HOME/.adeel-dev/evals/` with auto-comparison
...
```
adeel/
├── skills/          # Shipped skill directories
│   ├── browse/      # Headless browser CLI skill
│   │   ├── src/
│   ├── adeel-update/
│   ├── adeel-upgrade/
│   ├── autoplan/
...
├── test/            # Validation and eval tests
├── hooks/           # Plugin hooks
├── .claude-plugin/  # Plugin metadata
├── setup            # One-time setup helper
├── SKILL.md         # Generated from SKILL.md.tmpl (do not edit directly)
├── SKILL.md.tmpl    # Template source for SKILL.md
...
2. **If missing, AskUserQuestion** — let the user tell you or let adeel search the repo
3. **Persist the answer to CLAUDE.md** so we never have to ask again

This applies to test commands, eval commands, deploy commands, and any other
project-specific behavior. The project owns its config; adeel reads it.
...
`/adeel:browse` skill or run the browse binary directly via `$B <command>`. NEVER use
`mcp__claude-in-chrome__*` tools — they are slow, unreliable, and not what this
project uses.

## Vendored symlink awareness

When developing adeel, `.claude/skills/adeel` may be a symlink back to this
working directory (gitignored). This means skill changes are **live immediately** —
great for rapid iteration, risky during big refactors where half-written skills
could break other Claude Code sessions using adeel concurrently.

**Check once per session:** Run `ls -la .claude/skills/adeel` to see if it's a
symlink or a real copy. If it's a symlink to your working directory, be aware that:
- Template changes + `bun run gen:skill-docs` immediately affect all adeel invocations
- Breaking changes to SKILL.md.tmpl files can break concurrent adeel sessions
- During large refactors, remove the symlink (`rm .claude/skills/adeel`) so the
  global install at `${CLAUDE_PLUGIN_ROOT}/` is used instead

**Prefix setting:** Skill symlinks use either short names (`qa -> adeel/qa`) or
namespaced (`adeel-qa -> adeel/qa`), controlled by `skill_prefix` in
`$HOME/.adeel/config.yaml`. When vendoring into a project, run `./setup` after
...
before going live (especially if the user is actively using adeel in other windows).
...
- At `/adeel:ship` time (Step 5), not during development or mid-branch.
...
When estimating or discussing effort, always show both human-team and CC+adeel time:
...
Contributors can store long-range vision docs and design documents in `$HOME/.adeel-dev/plans/`.
These are local-only (not checked in). When reviewing TODOS.md, check `plans/` for candidates
that may be ready to promote to TODOs or implement.

## E2E eval failure blame protocol

When an E2E eval fails during `/adeel:ship` or any other workflow, **never claim "not
...
The active plugin lives at `${CLAUDE_PLUGIN_ROOT}/`. After making changes:

1. Push your branch
2. Fetch and reset in the plugin directory: `cd ${CLAUDE_PLUGIN_ROOT} && git fetch origin && git reset --hard origin/main`
3. Rebuild: `cd ${CLAUDE_PLUGIN_ROOT} && bun run build`
