// adeel telemetry-ingest edge function
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const event of events) {
      // Required fields
      if (!event.ts || !event.adeel_version || !event.os || !event.outcome) {
      ...
      rows.push({
        ...
        adeel_version: String(event.adeel_version).slice(0, 20),
