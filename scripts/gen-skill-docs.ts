...
function processTemplate(tmplPath: string, host: Host = 'claude'): { outputPath: string; content: string } {
  ...
  const skillDir = path.relative(SKILLS_ROOT, path.dirname(tmplPath));
  ...
  if (host === 'codex') {
    ...
    content = content.replace(/~\/\.claude\/skills\/adeel/g, ctx.paths.skillRoot);
    content = content.replace(/\.claude\/skills\/adeel/g, ctx.paths.localSkillRoot);
    content = content.replace(/\.claude\/skills\/review/g, '.agents/skills/adeel/review');
...
for (const tmplPath of findTemplates()) {
  // Skip /adeel:codex skill for codex host (self-referential — it's a Claude wrapper around codex exec)
  if (HOST === 'codex') {
