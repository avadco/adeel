...
export function codexSkillName(skillDir: string): string {
  if (skillDir === '.' || skillDir === '') return 'adeel';
  // Don't double-prefix: adeel-upgrade → adeel-upgrade (not adeel-adeel-upgrade)
  if (skillDir.startsWith('adeel-')) return skillDir;
  return `adeel-${skillDir}`;
}
