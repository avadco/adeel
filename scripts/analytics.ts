#!/usr/bin/env bun
/**
 * analytics — CLI for viewing adeel skill usage statistics.
 *
 * Reads $HOME/.adeel/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.adeel', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('adeel skill usage analytics');
