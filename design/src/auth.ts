 ...
 *
 * Resolution order:
 * 1. $HOME/.adeel/openai.json → { "api_key": "sk-..." }
...
const CONFIG_PATH = path.join(process.env.HOME || "~", ".adeel", "openai.json");

export function resolveApiKey(): string | null {
  // 1. Check $HOME/.adeel/openai.json
  try {
...
/**
 * Save an API key to $HOME/.adeel/openai.json with 0600 permissions.
 */
...
export function requireApiKey(): string {
  ...
  if (!key) {
    ...
    console.error("  or save to $HOME/.adeel/openai.json: { \"api_key\": \"sk-...\" }");
