...
When Claude controls your real browser via CDP (adeel `$B connect`), you look at two
windows: **Conductor** (to see Claude's thinking) and **Chrome** (to see Claude's actions).

adeel's Chrome extension Side Panel shows browse activity — every command, result,
...
A "Session" tab in the adeel Chrome extension Side Panel that shows:
  ...
  "workspaces": [
    {
      "id": "abc123",
      "name": "adeel",
      "branch": "user/chrome-extension-ctrl",
      "directory": "/Users/user/adeel",
      "pid": 12345,
      "active": true
    }
  ]
...
## What adeel builds (extension side)
