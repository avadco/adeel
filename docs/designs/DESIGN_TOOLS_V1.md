# Design: adeel Visual Design Generation (`design` binary)

Generated by /adeel:office-hours on 2026-03-26
...
A compiled TypeScript binary (`design/dist/design`) that wraps the OpenAI Images/Responses API, callable from skill templates via `$D` (mirroring the existing `$B` browse binary pattern). Priority integration order: /adeel:office-hours → /adeel:plan-design-review → /adeel:design-consultation → /adeel:design-review.
...
4. Priority: /adeel:office-hours first, then /adeel:plan-design-review, /adeel:design-consultation, /adeel:design-review.
...
- Scoped 48-hour prototype: shared `visual_mockup.ts` utility, /adeel:office-hours + /adeel:plan-design-review only, hero mockup + 2 variants
...
### Comparison Board Design Spec (from /adeel:plan-design-review)
...
**Screenshot consent (first-time only for $D evolve):** "This will send a screenshot of your live site to OpenAI for design evolution. [Proceed] [Don't ask again]" Stored in $HOME/.adeel/config.yaml as design_screenshot_consent.
...
- Exploration variants go to `/tmp/adeel-mockups-{session}/` (ephemeral, not committed)
...
- Fallback: if not in a git repo, save to `/tmp/adeel-mockup-{timestamp}.png`
...
```typescript
// claude host:
designDir: '${CLAUDE_PLUGIN_ROOT}/design/dist'
// codex host:
designDir: '$ADEEL_DESIGN'
```
Note: Codex runtime setup (`setup` script) must also export `ADEEL_DESIGN` env var, similar to how `ADEEL_BROWSE` is set.
...
**1. /adeel:office-hours** — Replace the Visual Sketch section
...
**2. /adeel:plan-design-review** — "What better looks like"
...
**3. /adeel:design-consultation** — Design system preview
...
**4. /adeel:design-review** — Design intent comparison
...
| `scripts/resolvers/preamble.ts` | Add `ADEEL_DESIGN` env var export for Codex host |
...
1. Read `$HOME/.adeel/openai.json` → `{ "api_key": "sk-..." }` (file permissions 0600)
...
3. If neither exists → guided setup flow:
   ...
   - Write to `$HOME/.adeel/openai.json` with 0600 permissions
...
## CEO Expansion Scope (accepted via /adeel:plan-ceo-review SCOPE EXPANSION)
...
- Used in: /adeel:design-review, iteration feedback, PR review
...
- Bridge between /adeel:design-review critique and visual fix proposal

### 4. Design Intent Verification
- During /adeel:design-review, overlay approved mockup (docs/designs/) onto live screenshot
...
- Running `/adeel:office-hours` on a UI idea produces actual PNG mockups alongside the design doc
- Running `/adeel:plan-design-review` shows "what better looks like" as a mockup, not prose
...
- Symlinked via existing `${CLAUDE_PLUGIN_ROOT}/` install path
...
- Auth module (read $HOME/.adeel/openai.json, fallback to env var, guided setup flow)
...
- Add ADEEL_DESIGN env var export to `scripts/resolvers/preamble.ts` (Codex host)
- Update `test/gen-skill-docs.test.ts` (DESIGN_SKETCH test suite)
- Regenerate SKILL.md files

### Commit 4: /adeel:office-hours integration
...
### Commit 5: /adeel:plan-design-review integration
...
- Integration into /adeel:design-review template: auto-verify when approved mockup exists
...
5. Running `/adeel:office-hours` on a UI idea produces mockups inline
...
- You prioritized /adeel:office-hours first — the upstream leverage point. If the brainstorm produces real mockups, every downstream skill (/adeel:plan-design-review, /adeel:design-review) has a visual artifact to reference instead of re-interpreting prose.
...
|--------|---------|-----|------|--------|----------|
| Office Hours | `/adeel:office-hours` | Design brainstorm | 1 | DONE | 4 premises, 1 revised (Codex: opt-in->default-on) |
| CEO Review | `/adeel:plan-ceo-review` | Scope & strategy | 1 | CLEAR | EXPANSION: 6 proposed, 6 accepted, 0 deferred |
| Eng Review | `/adeel:plan-eng-review` | Architecture & tests (required) | 1 | CLEAR | 7 issues, 0 critical gaps, 4 outside voices |
| Design Review | `/adeel:plan-design-review` | UI/UX gaps | 1 | CLEAR | score: 2/10 -> 8/10, 5 decisions made |
