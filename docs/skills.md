...
Detailed guides for every adeel skill — philosophy, workflow, and examples.

| Skill | Your specialist | What they do |
|-------|----------------|--------------|
| [`/adeel:office-hours`](#office-hours) | **YC Office Hours** | Start here. Six forcing questions that reframe your product before you write code. Pushes back on your framing, challenges premises, generates implementation alternatives. Design doc feeds into every downstream skill. |
| [`/adeel:plan-ceo-review`](#plan-ceo-review) | **CEO / Founder** | Rethink the problem. Find the 10-star product hiding inside the request. Four modes: Expansion, Selective Expansion, Hold Scope, Reduction. |
| [`/adeel:plan-eng-review`](#plan-eng-review) | **Eng Manager** | Lock in architecture, data flow, diagrams, edge cases, and tests. Forces hidden assumptions into the open. |
| [`/adeel:plan-design-review`](#plan-design-review) | **Senior Designer** | Interactive plan-mode design review. Rates each dimension 0-10, explains what a 10 looks like, fixes the plan. Works in plan mode. |
| [`/adeel:design-consultation`](#design-consultation) | **Design Partner** | Build a complete design system from scratch. Knows the landscape, proposes creative risks, generates realistic product mockups. Design at the heart of all other phases. |
| [`/adeel:review`](#review) | **Staff Engineer** | Find the bugs that pass CI but blow up in production. Auto-fixes the obvious ones. Flags completeness gaps. |
| [`/adeel:investigate`](#investigate) | **Debugger** | Systematic root-cause debugging. Iron Law: no fixes without investigation. Traces data flow, tests hypotheses, stops after 3 failed fixes. |
| [`/adeel:design-review`](#design-review) | **Designer Who Codes** | Live-site visual audit + fix loop. 80-item audit, then fixes what it finds. Atomic commits, before/after screenshots. |
| [`/adeel:qa`](#qa) | **QA Lead** | Test your app, find bugs, fix them with atomic commits, re-verify. Auto-generates regression tests for every fix. |
| [`/adeel:qa-only`](#qa) | **QA Reporter** | Same methodology as /adeel:qa but report only. Use when you want a pure bug report without code changes. |
| [`/adeel:ship`](#ship) | **Release Engineer** | Sync main, run tests, audit coverage, push, open PR. Bootstraps test frameworks if you don't have one. One command. |
| [`/adeel:cso`](#cso) | **Chief Security Officer** | OWASP Top 10 + STRIDE threat modeling security audit. Scans for injection, auth, crypto, and access control issues. |
| [`/adeel:document-release`](#document-release) | **Technical Writer** | Update all project docs to match what you just shipped. Catches stale READMEs automatically. |
| [`/adeel:retro`](#retro) | **Eng Manager** | Team-aware weekly retro. Per-person breakdowns, shipping streaks, test health trends, growth opportunities. |
| [`/adeel:browse`](#browse) | **QA Engineer** | Give the agent eyes. Real Chromium browser, real clicks, real screenshots. ~100ms per command. |
| [`/adeel:setup-browser-cookies`](#setup-browser-cookies) | **Session Manager** | Import cookies from your real browser (Chrome, Arc, Brave, Edge) into the headless session. Test authenticated pages. |
| | | |
| **Multi-AI** | | |
| [`/adeel:codex`](#codex) | **Second Opinion** | Independent review from OpenAI Codex CLI. Three modes: code review (pass/fail gate), adversarial challenge, and open consultation with session continuity. Cross-model analysis when both `/adeel:review` and `/adeel:codex` have run. |
| | | |
| **Safety & Utility** | | |
| [`/adeel:careful`](#safety--guardrails) | **Safety Guardrails** | Warns before destructive commands (rm -rf, DROP TABLE, force-push, git reset --hard). Override any warning. Common build cleanups whitelisted. |
| [`/adeel:freeze`](#safety--guardrails) | **Edit Lock** | Restrict all file edits to a single directory. Blocks Edit and Write outside the boundary. Accident prevention for debugging. |
| [`/adeel:guard`](#safety--guardrails) | **Full Safety** | Combines /adeel:careful + /adeel:freeze in one command. Maximum safety for prod work. |
| [`/adeel:unfreeze`](#safety--guardrails) | **Unlock** | Remove the /adeel:freeze boundary, allowing edits everywhere again. |
| [`/adeel-upgrade`](#adeel-upgrade) | **Self-Updater** | Upgrade adeel to the latest version. Detects global vs vendored install, syncs both, shows what changed. |
...
Both modes end with a design doc written to `$HOME/.adeel/projects/` — and that doc feeds directly into `/adeel:plan-ceo-review` and `/adeel:plan-eng-review`. The full lifecycle is now: `office-hours → plan → implement → review → QA → ship → retro`.

After the design doc is approved, `/adeel:office-hours` reflects on what it noticed about how you think — not generic praise, but specific callbacks to things you said during the session. The observations appear in the design doc too, so you re-encounter them when you re-read later.

---

## `/adeel:plan-ceo-review`
...
In `/adeel:plan-ceo-review`, I want the model to ask whether "photo upload" is even the feature. Maybe the real feature is helping someone create a listing that actually sells.
...
That is what `/adeel:plan-ceo-review` does for me.
...
Visions and decisions are persisted to `$HOME/.adeel/projects/` so they survive beyond the conversation. Exceptional visions can be promoted to `docs/designs/` in your repo for the team.

---

## `/adeel:plan-eng-review`
...
So `/adeel:plan-eng-review` is where I want the model to build the technical spine that can carry the product vision.

### Example

Take the same listing app example.

Let's say `/adeel:plan-ceo-review` already did its job. We decided the real feature is not just photo upload. It is a smart listing flow that:
...
Now `/adeel:plan-eng-review` takes over.
...
Eng Review is the only required gate (disable with `adeel-config set skip_eng_review true`). CEO and Design are informational — recommended for product and UI changes respectively.

### Plan-to-QA flow

When `/adeel:plan-eng-review` finishes the test review section, it writes a test plan artifact to `$HOME/.adeel/projects/`. When you later run `/adeel:qa`, it picks up that test plan automatically — your engineering review feeds directly into QA testing with no manual copy-paste.

---

## `/adeel:plan-design-review`
...
`/adeel:plan-design-review` catches all of this during planning, when it's cheap to fix.

It works like `/adeel:plan-ceo-review` and `/adeel:plan-eng-review` — interactive, one issue at a time, with the **STOP + AskUserQuestion** pattern. It rates each design dimension 0-10, explains what a 10 looks like, then edits the plan to get there. The rating drives the work: rate low = lots of fixes, rate high = quick pass.
...
Claude: Initial Design Rating: 4/10
        ...
        "Plan is design-complete. Run /adeel:design-review after
         implementation for visual QA."
```

When you re-run it, sections already at 8+ get a quick pass. Sections below 8 get full treatment. For live-site visual audits post-implementation, use `/adeel:design-review`.

---

## `/adeel:design-consultation`

This is my **design partner mode**.

`/adeel:plan-design-review` audits a site that already exists. `/adeel:design-consultation` is for when you have nothing yet — no design system, no font choices, no color palette. You are starting from zero and you want a senior designer to sit down with you and build the whole visual identity together.
...
Then it writes `DESIGN.md` to your repo root — your project's design source of truth — and updates `CLAUDE.md` so every future Claude Code session respects the system. From that point on, `/adeel:design-review` can audit against it, and any agent working on your frontend knows the rules.

### Example

```
You:   /adeel:design-consultation
...
`/adeel:plan-design-review` reviews your plan before implementation. `/adeel:design-review` audits and fixes the live site after.
...
```
You:   /adeel:design-review https://myapp.com
...
Claude: [Runs full 80-item visual audit on the live site]
        ...
        [Report with before/after screenshots saved to .adeel/design-reports/]
```
...
`/adeel:review` exists because there is a whole class of bugs that can survive CI and still punch you in the face in production. This mode is not about dreaming bigger. It is not about making the plan prettier. It is about asking:
...
* forgotten enum handlers — add a new status or type constant, and `/adeel:review` traces it through every switch statement and allowlist in your codebase, not just the files you changed
...
`/adeel:review` now flags shortcut implementations where the complete version costs less than 30 minutes of CC time. If you chose the 80% solution and the 100% solution is a lake, not an ocean, the review will call it out.

### Example

Suppose the smart listing flow is implemented and the tests are green.

`/adeel:review` should still ask:
...
That is the point of `/adeel:review`.
...
When something is broken and you don't know why, `/adeel:investigate` is your systematic debugger. It follows the Iron Law: **no fixes without root cause investigation first.**
...
`/adeel:browse` gives the agent eyes. `/adeel:qa` gives it a testing methodology.

The most common use case: you're on a feature branch, you just finished coding, and you want to verify everything works. Just say `/adeel:qa` — it reads your git diff, identifies which pages and routes your changes affect, spins up the browser, and tests each one. No URL required. No manual test plan.
...
When `/adeel:qa` fixes a bug and verifies it, it automatically generates a regression test that catches the exact scenario that broke. Tests include full attribution tracing back to the QA report.

### Example

```
You:   /adeel:qa https://staging.myapp.com
...
Claude: [Explores 12 pages, fills 3 forms, tests 2 flows]
        ...
        [Full report with screenshots saved to .adeel/qa-reports/]
```

**Testing authenticated pages:** Use `/adeel:setup-browser-cookies` first to import your real browser sessions, then `/adeel:qa` can test pages behind login.
...
`/adeel:ship` is for the final mile. It is for a ready branch, not for deciding what to build.
...
If your project doesn't have a test framework, `/adeel:ship` sets one up — detects your runtime, researches the best framework, installs it, writes 3-5 real tests for your actual code, sets up CI/CD (GitHub Actions), and creates TESTING.md. 100% test coverage is the goal — tests make vibe coding safe instead of yolo coding.

### Coverage audit

Every `/adeel:ship` run builds a code path map from your diff, searches for corresponding tests, and produces an ASCII coverage diagram with quality stars. Gaps get tests auto-generated. Your PR body shows the coverage: `Tests: 42 → 47 (+5 new)`.

### Review gate

`/adeel:ship` checks the [Review Readiness Dashboard](#review-readiness-dashboard) before creating the PR. If the Eng Review is missing, it asks — but won't block you. Decisions are saved per-branch so you're never re-asked.
...
Run `/adeel:cso` on any codebase and it performs an OWASP Top 10 + STRIDE threat model audit. It scans for injection vulnerabilities, broken authentication, sensitive data exposure, XML external entities, broken access control, security misconfiguration, XSS, insecure deserialization, known-vulnerable components, and insufficient logging. Each finding includes severity, evidence, and a recommended fix.
...
## `/adeel:document-release`

This is my **technical writer mode**.

After `/adeel:ship` creates the PR but before it merges, `/adeel:document-release` reads every documentation file in the project and cross-references it against the diff. It updates file paths, command lists, project structure trees, and anything else that drifted. Risky or subjective changes get surfaced as questions — everything else is handled automatically.

```
You:   /adeel:document-release
...
At the end of the week I want to know what actually happened. Not vibes — data. `/adeel:retro` analyzes commit history, work patterns, and shipping velocity and writes a candid retrospective.
...
`/adeel:browse` is the skill that closes the loop. Before it, the agent could think and code but was still half blind. It had to guess about UI state, auth flows, redirects, console errors, empty states, and broken layouts. Now it can just go look.
...
```
You:   /adeel:browse staging.myapp.com — log in, test the signup flow, and check
...
**Security note:** `/adeel:browse` runs a persistent Chromium session. Cookies, localStorage, and session state carry over between commands. Do not use it against sensitive production environments unless you intend to — it is a real browser with real state. The session auto-shuts down after 30 minutes of idle time.

For the full command reference, see [BROWSER.md](../BROWSER.md).

---

## `/adeel:setup-browser-cookies`

This is my **session manager mode**.

Before `/adeel:qa` or `/adeel:browse` can test authenticated pages, they need cookies. Instead of manually logging in through the headless browser every time, `/adeel:setup-browser-cookies` imports your real sessions directly from your daily browser.
...
```
You:   /adeel:setup-browser-cookies
...
```
You:   /adeel:setup-browser-cookies github.com
...
When both `/adeel:review` (Claude) and `/adeel:codex` (OpenAI) have reviewed the same branch, you get a cross-model comparison: which findings overlap (high confidence), which are unique to Codex (different perspective), and which are unique to Claude. This is the "two doctors, same patient" approach to code review.

```
You:   /adeel:codex review
...
Claude: Running independent Codex review...
        ...
        Cross-model analysis (vs /adeel:review):
...
Say "be careful" or run `/adeel:careful` when you're working near production, running destructive commands, or just want a safety net. Every Bash command gets checked against known-dangerous patterns:
...
Restrict all file edits to a single directory. When you're debugging a billing bug, you don't want Claude accidentally "fixing" unrelated code in `src/auth/`. `/adeel:freeze src/billing` blocks all Edit and Write operations outside that path.

`/adeel:investigate` activates this automatically — it detects the module being debugged and freezes edits to that directory.

```
You:   /adeel:freeze src/billing

Claude: Edits restricted to src/billing/. Run /adeel:unfreeze to remove.
...
Full safety mode — combines `/adeel:careful` + `/adeel:freeze` in one command. Destructive command warnings plus directory-scoped edits. Use when touching prod or debugging live systems.

### `/adeel:unfreeze`

Remove the `/adeel:freeze` boundary, allowing edits everywhere again. The hooks stay registered for the session — they just allow everything. Run `/adeel:freeze` again to set a new boundary.

---

## `/adeel-upgrade`

Keep adeel current with one command. It detects your install type (global at `~/.claude/skills/adeel` vs vendored in your project at `.claude/skills/adeel`), runs the upgrade, syncs both copies if you have dual installs, and shows you what changed.
...
Claude: Current version: 0.7.4
        ...
        - /adeel:codex multi-AI second opinion
        - /adeel:qa always uses browser now
        - Safety skills: /adeel:careful, /adeel:freeze, /adeel:guard
        - Proactive skill suggestions

        Upgraded to 0.8.2. Both global and project installs synced.
```

Set `auto_upgrade: true` in `$HOME/.adeel/config.yaml` to skip the prompt entirely — adeel upgrades silently at the start of each session when a new version is available.
...
Install Greptile on your GitHub repo at [greptile.com](https://greptile.com) — it takes about 30 seconds. Once it's reviewing your PRs, adeel picks up its comments automatically. No additional configuration.
...
adeel solves this. `/adeel:review` and `/adeel:ship` are now Greptile-aware. They read Greptile's comments, classify each one, and take action:
...
The result is a two-layer review: Greptile catches things asynchronously on the PR, then `/adeel:review` and `/adeel:ship` triage those findings as part of the normal workflow. Nothing falls through the cracks.

### Learning from history

Every false positive you confirm gets saved to `$HOME/.adeel/greptile-history.md`. Future runs auto-skip known FP patterns for your codebase. And `/adeel:retro` tracks Greptile's batting average over time — so you can see whether the signal-to-noise ratio is improving.
