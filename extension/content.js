...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'adeel-status-pill';
  ...
  statusPill.innerHTML = `<span class="adeel-pill-dot"></span> adeel${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'adeel-ref-overlays';
...
function renderRefBadges(refs) {
  ...
  for (const ref of refs) {
    ...
    badge.className = 'adeel-ref-badge';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'adeel-ref-panel';

  const header = document.createElement('div');
  header.className = 'adeel-ref-panel-header';
  header.textContent = `adeel refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'adeel-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'adeel-ref-panel-row';
    row.innerHTML = `<span class="adeel-ref-panel-id">${ref.ref}</span> <span class="adeel-ref-panel-role">${ref.role}</span> <span class="adeel-ref-panel-name">"${ref.name}"</span>`;
    list.appendChild(row);
  }
  if (refs.length > 30) {
    const more = document.createElement('div');
    more.className = 'adeel-ref-panel-more';
