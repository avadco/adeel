...
describe('Cross-skill path consistency', () => {
  ...
  test('all greptile-history write references specify both per-project and global paths', () => {
    ...
    for (const file of filesToCheck) {
      ...
      const hasBoth = (content.includes('per-project') && content.includes('global')) ||
        (content.includes('$REMOTE_SLUG/greptile-history') && content.includes('$HOME/.adeel/greptile-history'));
  ...
  test('greptile-triage.md contains both project and global history paths', () => {
    ...
    expect(content).toContain('$HOME/.adeel/greptile-history.md');
  });
...
describe('QA skill structure validation', () => {
  ...
  test('output structure references report directory layout', () => {
    ...
    expect(qaContent).toContain('.adeel/qa-reports/');
  });
});
...
describe('office-hours skill structure', () => {
  ...
  test('contains YC apply CTA with ref tracking', () => {
    expect(content).toContain('ycombinator.com/apply?ref=adeel');
  });
  ...
  test('contains User Name personal plea', () => {
    expect(content).toContain('User Name, the creator of adeel');
  });
...
describe('Contributor mode preamble structure', () => {
  ...
  for (const skill of skillsWithPreamble) {
    ...
    test(`${skill} uses periodic reflection (not per-command)`, () => {
      ...
      expect(content).not.toContain('After you use adeel-provided CLIs');
    });
  }
});
...
describe('Completeness Principle in generated SKILL.md files', () => {
  ...
  for (const skill of skillsWithPreamble) {
    test(`${skill} contains Completeness Principle section`, () => {
      ...
      expect(content).toContain('Go all the way');
    });
  }
  ...
  test('Completeness Principle includes compression table in tier 2+ skills', () => {
    ...
    expect(content).toContain('CC+adeel');
...
describe('CEO review mode validation', () => {
  ...
  test('contains prerequisite skill offer for office-hours', () => {
    ...
    expect(content).toContain('/adeel:office-hours');
  });
...
describe('adeel-slug', () => {
  const SLUG_BIN = path.join(ROOT, 'bin', 'adeel-slug');
  ...
  test('eval sets variables under bash with set -euo pipefail', () => {
    const result = Bun.spawnSync(
      ['bash', '-c', 'set -euo pipefail; eval "$(./bin/adeel-slug 2>/dev/null)"; echo "SLUG=$SLUG"; echo "BRANCH=$BRANCH"'],
  ...
  test('no templates or bin scripts use source process substitution for adeel-slug', () => {
    const result = Bun.spawnSync(
      ['grep', '-r', 'source <(.*adeel-slug', '--include=*.tmpl', '--include=adeel-review-*', '.'],
      { cwd: ROOT, stdout: 'pipe', stderr: 'pipe' }
    );
...
describe('Test Bootstrap ({{TEST_BOOTSTRAP}}) integration', () => {
  ...
  test('TEST_BOOTSTRAP does NOT appear in qa-only/SKILL.md', () => {
    ...
    expect(content).toContain('Run `/adeel:qa` to bootstrap');
  });
...
describe('Phase 8e.5 regression test generation', () => {
  ...
  test('regression test includes full attribution comment format', () => {
    ...
    expect(content).toContain('// Found by /adeel:qa on');
    expect(content).toContain('// Report: .adeel/qa-reports/');
  });
...
describe('Codex skill', () => {
  ...
  test('codex/SKILL.md contains review log persistence', () => {
    ...
    expect(content).toContain('adeel-review-log');
  });
  ...
  test('adversarial review in /adeel:review auto-scales by diff size', () => {
  ...
  test('adversarial review in /adeel:ship auto-scales by diff size', () => {
    const content = fs.readFileSync(path.join(ROOT, 'ship', 'SKILL.md'), 'utf-8');
    expect(content).toContain('Adversarial review (auto-scaled)');
    expect(content).toContain('< 50');
    expect(content).toContain('200+');
  ...
  test('codex-host ship/review do NOT contain adversarial review step', () => {
    ...
    const shipContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'adeel-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'adeel-review', 'SKILL.md'), 'utf-8');
  ...
  test('codex integration in /adeel:plan-eng-review offers plan critique', () => {
    const content = fs.readFileSync(path.join(ROOT, 'plan-eng-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('Codex');
    expect(content).toContain('codex exec');
  });

  test('/adeel:review persists a review-log entry for ship readiness', () => {
    const content = fs.readFileSync(path.join(ROOT, 'review', 'SKILL.md'), 'utf-8');
...
describe('Skill trigger phrases', () => {
  ...
  // Excluded: root adeel (browser tool), adeel-upgrade (adeel-specific),
  // humanizer (text tool)
...
describe('Codex skill validation', () => {
  ...
  // Discover all Claude skills with templates (except /adeel:codex which is Claude-only)
  ...
  test('all skills (except /adeel:codex) have both Claude and Codex variants', () => {
    for (const skillDir of CLAUDE_SKILLS_WITH_TEMPLATES) {
      ...
      // Codex variant
      const codexName = skillDir.startsWith('adeel-') ? skillDir : `adeel-${skillDir}`;
  ...
  test('/adeel:codex skill is Claude-only — no Codex variant', () => {
  ...
  test('Codex skill names follow adeel-{name} convention', () => {
    const codexDirs = fs.readdirSync(AGENTS_DIR);
    for (const dir of codexDirs) {
      // Every directory should start with adeel
      expect(dir.startsWith('adeel')).toBe(true);
      // Root is just 'adeel', others are 'adeel-{name}'
      if (dir !== 'adeel') {
        expect(dir.startsWith('adeel-')).toBe(true);
      }
    }
...
describe('Repo mode preamble validation', () => {
  test('generated SKILL.md preamble contains REPO_MODE output', () => {
    ...
    expect(content).toContain('adeel-repo-mode');
  });
