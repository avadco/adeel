...
Write your review findings to ${reviewDir}/review-output.md`,
    ...
    recordE2E(evalCollector, '/adeel:review SQL injection', 'Review skill E2E', result);
...
The diff adds a new "returned" status to the Order model. Your job is to check if all consumers handle it.`,
    ...
    recordE2E(evalCollector, '/adeel:review enum completeness', 'Review enum completeness E2E', result);
    expect(result.exitReason).toBe('success');
...
Important: The design checklist should catch issues like blacklisted fonts, small font sizes, outline:none, !important, AI slop patterns (purple gradients, generic hero copy, 3-column feature grid), etc.`,
    ...
    recordE2E(evalCollector, '/adeel:review design lite', 'Review design lite E2E', result);
    expect(result.exitReason).toBe('success');
...
Write your findings to ${dir}/review-output.md`,
    ...
    recordE2E(evalCollector, '/adeel:review base branch detection', 'Base branch detection', result);
    expect(result.exitReason).toBe('success');
...
- The diff stat against the base branch`,
    ...
    recordE2E(evalCollector, '/adeel:ship base branch detection', 'Base branch detection', result);
    expect(result.exitReason).toBe('success');
...
Write your retrospective to ${dir}/retro-output.md`,
    ...
    recordE2E(evalCollector, '/adeel:retro default branch detection', 'Base branch detection', result, {
...
Analyze the git history and produce the narrative report as described in the SKILL.md.`,
    ...
    recordE2E(evalCollector, '/adeel:retro', 'Retro E2E', result, {
      passed: ['success', 'error_max_turns'].includes(result.exitReason),
    });
...
describeIfSelected('Review Dashboard Via Attribution', ['review-dashboard-via'], () => {
  ...
  beforeAll(() => {
    ...
    // adeel-review-read reads from $HOME/.adeel/projects/$SLUG/$BRANCH-reviews.jsonl
    // For the test, we'll write a mock adeel-review-read script that returns our test data
    ...
    // Write a mock adeel-review-read that returns our test data
    const mockBinDir = path.join(dashDir, '.mock-bin');
    fs.mkdirSync(mockBinDir, { recursive: true });
    fs.writeFileSync(path.join(mockBinDir, 'adeel-review-read'), [
      '#!/usr/bin/env bash',
    ...
    fs.chmodSync(path.join(mockBinDir, 'adeel-review-read'), 0o755);
...
Instead of running ${CLAUDE_PLUGIN_ROOT}/bin/adeel-review-read, run this mock: ${mockBinDir}/adeel-review-read

Parse the output and display the dashboard table. Pay attention to:
1. The "via" field in entries — show source attribution (e.g., "via /adeel:autoplan")
...
Write the dashboard output to ${dashDir}/dashboard-output.md`,
    ...
    logCost('/adeel:ship dashboard-via', result);
    recordE2E(evalCollector, '/adeel:ship review dashboard via attribution', 'Dashboard via field', result);
    expect(result.exitReason).toBe('success');
