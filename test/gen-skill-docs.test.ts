...
const ALL_SKILLS = (() => {
  ...
  if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
    skills.push({ dir: '.', name: 'root adeel' });
  }
...
describe('gen-skill-docs', () => {
  ...
  test('generated SKILL.md contains contributor mode check', () => {
    ...
    expect(content).toContain('adeel_contributor');
  ...
  test('generated SKILL.md contains telemetry line', () => {
    ...
    expect(content).toContain('$HOME/.adeel/analytics');
  });
  ...
  test('preamble .pending-* glob is zsh-safe (uses find, not shell glob)', () => {
    for (const skill of ALL_SKILLS) {
      ...
      expect(content).toContain("find $HOME/.adeel/analytics -maxdepth 1 -name '.pending-*'");
    }
  });
...
describe('REVIEW_DASHBOARD resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`review dashboard appears in ${skill} generated file`, () => {
      ...
      expect(content).toContain('adeel-review');
  ...
  test('plan-ceo-review chaining mentions eng and design reviews', () => {
    const content = fs.readFileSync(path.join(ROOT, 'plan-ceo-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('/adeel:plan-eng-review');
    expect(content).toContain('/adeel:plan-design-review');
  });

  test('plan-eng-review chaining mentions design and ceo reviews', () => {
    const content = fs.readFileSync(path.join(ROOT, 'plan-eng-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('/adeel:plan-design-review');
    expect(content).toContain('/adeel:plan-ceo-review');
  });
...
describe('PLAN_FILE_REVIEW_REPORT resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`plan file review report appears in ${skill} generated file`, () => {
      const content = fs.readFileSync(path.join(ROOT, skill, 'SKILL.md'), 'utf-8');
      expect(content).toContain('ADEEL REVIEW REPORT');
    });
  }
  ...
  test('resolver output contains key report elements', () => {
    ...
    expect(content).toContain('/adeel:plan-eng-review');
    expect(content).toContain('/adeel:plan-design-review');
    expect(content).toContain('/adeel:codex review');
  });
});
...
describe('PLAN_VERIFICATION_EXEC placeholder', () => {
  ...
  test('references /adeel:qa-only invocation', () => {
...
describe('Coverage gate in ship', () => {
  ...
  test('review SKILL.md contains coverage WARNING', () => {
    ...
    expect(reviewSkill).toContain('Consider writing tests before running /adeel:ship');
  });
...
describe('Plan status footer in preamble', () => {
  test('preamble contains plan status footer', () => {
    ...
    expect(content).toContain('ADEEL REVIEW REPORT');
    expect(content).toContain('adeel-review-read');
...
describe('CODEX_SECOND_OPINION resolver', () => {
  const content = fs.readFileSync(path.join(ROOT, 'office-hours', 'SKILL.md'), 'utf-8');
  const codexContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'adeel-office-hours', 'SKILL.md'), 'utf-8');
  ...
  test('Codex host variant does NOT contain the Phase 3.5 resolver output', () => {
    ...
    expect(codexContent).not.toContain('adeel-codex-oh-');
  });
});
...
describe('Codex filesystem boundary', () => {
  ...
  const CODEX_CALLING_SKILLS = [
    'codex',         // /adeel:codex skill — 3 modes
    'autoplan',      // /adeel:autoplan — CEO/design/eng voices
    'review',        // /adeel:review — adversarial step resolver
    'ship',          // /adeel:ship — adversarial step resolver
  ...
  test('codex skill has rabbit-hole detection rule', () => {
    ...
    expect(content).toContain('adeel-update-check');
  ...
  test('autoplan boundary text avoids host-specific paths for cross-host compatibility', () => {
    ...
    // autoplan template uses generic 'skills/adeel' pattern instead of host-specific
    ...
    expect(boundarySection).toContain('skills/adeel');
...
describe('Codex generation (--host codex)', () => {
  ...
  // Dynamic discovery of expected Codex skills: all templates except /adeel:codex
  const CODEX_SKILLS = (() => {
    const skills: Array<{ dir: string; codexName: string }> = [];
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      skills.push({ dir: '.', codexName: 'adeel' });
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      if (entry.name === 'codex') continue; // /adeel:codex is excluded from Codex output
      if (!fs.existsSync(path.join(ROOT, entry.name, 'SKILL.md.tmpl'))) continue;
      const codexName = entry.name.startsWith('adeel-') ? entry.name : `adeel-${entry.name}`;
  ...
  test('root adeel bundle has OpenAI metadata for Codex skill browsing', () => {
    ...
    expect(content).toContain('display_name: "adeel"');
    expect(content).toContain('Use $adeel to locate the bundled adeel skills.');
  });

  test('codexSkillName mapping: root is adeel, others are adeel-{dir}', () => {
    // Root → adeel
    expect(fs.existsSync(path.join(AGENTS_DIR, 'adeel', 'SKILL.md'))).toBe(true);
    // Subdirectories → adeel-{dir}
    expect(fs.existsSync(path.join(AGENTS_DIR, 'adeel-review', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'adeel-ship', 'SKILL.md'))).toBe(true);
    // adeel-upgrade doesn't double-prefix
    expect(fs.existsSync(path.join(AGENTS_DIR, 'adeel-upgrade', 'SKILL.md'))).toBe(true);
    // No double-prefix: adeel-adeel-upgrade must NOT exist
    expect(fs.existsSync(path.join(AGENTS_DIR, 'adeel-adeel-upgrade', 'SKILL.md'))).toBe(false);
  });
  ...
  test('/adeel:codex skill excluded from Codex output', () => {
    expect(fs.existsSync(path.join(AGENTS_DIR, 'adeel-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'adeel-codex'))).toBe(false);
  });

  test('Codex review step stripped from Codex-host ship and review', () => {
    const shipContent = fs.readFileSync(path.join(AGENTS_DIR, 'adeel-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(AGENTS_DIR, 'adeel-review', 'SKILL.md'), 'utf-8');
  ...
  test('multiline descriptions preserved in Codex output', () => {
    ...
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'adeel-office-hours', 'SKILL.md'), 'utf-8');
    const fmEnd = content.indexOf('\n---', 4);
    const frontmatter = content.slice(4, fmEnd);
  ...
  test('hook skills have safety prose and no hooks: in frontmatter', () => {
    const HOOK_SKILLS = ['adeel-careful', 'adeel-freeze', 'adeel-guard'];
  ...
  test('Codex preamble resolves runtime assets from repo-local or global adeel roots', () => {
    // Check a skill that has a preamble (review is a good candidate)
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'adeel-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('ADEEL_ROOT');
    expect(content).toContain('$_ROOT/.agents/skills/adeel');
    expect(content).toContain('$ADEEL_BIN/adeel-config');
    expect(content).toContain('$ADEEL_ROOT/adeel-upgrade/SKILL.md');
    expect(content).not.toContain('~/.codex/skills/adeel/bin/adeel-config get telemetry');
  });

  // ─── Path rewriting regression tests ─────────────────────────

  test('sidecar paths point to .agents/skills/adeel/review/ (not adeel-review/)', () => {
    // Regression: gen-skill-docs rewrote .claude/skills/review → .agents/skills/adeel-review
    // but setup puts sidecars under .agents/skills/adeel/review/. Must match setup layout.
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'adeel-review', 'SKILL.md'), 'utf-8');
    // Correct: references to sidecar files use adeel/review/ path
    expect(content).toContain('.agents/skills/adeel/review/checklist.md');
    expect(content).toContain('.agents/skills/adeel/review/design-checklist.md');
    // Wrong: must NOT reference adeel-review/checklist.md (file doesn't exist there)
    expect(content).not.toContain('.agents/skills/adeel-review/checklist.md');
    expect(content).not.toContain('.agents/skills/adeel-review/design-checklist.md');
  });

  test('sidecar paths in ship skill point to adeel/review/ for pre-landing review', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'adeel-ship', 'SKILL.md'), 'utf-8');
    // Ship references the review checklist in its pre-landing review step
    if (content.includes('checklist.md')) {
      expect(content).toContain('.agents/skills/adeel/review/');
      expect(content).not.toContain('.agents/skills/adeel-review/checklist');
    }
  });
  ...
  test('greptile-triage sidecar path is correct', () => {
    ...
    if (content.includes('greptile-triage')) {
      expect(content).toContain('.agents/skills/adeel/review/greptile-triage.md');
      expect(content).not.toContain('.agents/skills/adeel-review/greptile-triage');
    }
  });
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 1: ~/.claude/skills/adeel → $ADEEL_ROOT
    expect(content).not.toContain('~/.claude/skills/adeel');
    expect(content).toContain('$ADEEL_ROOT');

    // Rule 2: .claude/skills/adeel → .agents/skills/adeel
    expect(content).not.toContain('.claude/skills/adeel');

    // Rule 3: .claude/skills/review → .agents/skills/adeel/review
  ...
  test('path rewrite rules apply to all Codex skills with sidecar references', () => {
    ...
    for (const skill of CODEX_SKILLS) {
      ...
      if (content.includes('checklist.md') && !content.includes('design-checklist.md')) {
        expect(content).not.toContain('adeel-review/checklist.md');
      }
    }
  });
  ...
  test('Claude output unchanged: review skill still uses .claude/skills/ paths', () => {
    ...
    expect(content).toContain('~/.claude/skills/adeel');
  ...
  test('Claude output unchanged: all Claude skills have zero Codex paths', () => {
    for (const skill of ALL_SKILLS) {
      const content = fs.readFileSync(path.join(ROOT, skill.dir, 'SKILL.md'), 'utf-8');
      expect(content).not.toContain('~/.codex/');
      // adeel-upgrade legitimately references .agents/skills for cross-platform detection
      if (skill.dir !== 'adeel-upgrade') {
        expect(content).not.toContain('.agents/skills');
      }
    }
  });
  ...
  test('codex host produces empty outside voices in design-review', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'adeel-design-review', 'SKILL.md'), 'utf-8');
    expect(codexContent).not.toContain('Design Outside Voices');
  });

  test('codex host does not include Codex design block in ship', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'adeel-ship', 'SKILL.md'), 'utf-8');
...
describe('setup script validation', () => {
  ...
  test('Codex install uses link_codex_skill_dirs', () => {
    ...
    expect(codexSection).not.toContain('ln -snf "$ADEEL_DIR" "$CODEX_ADEEL"');
  });
  ...
  test('setup separates install path from source path for symlinked repo-local installs', () => {
    expect(setupContent).toContain('INSTALL_ADEEL_DIR=');
    expect(setupContent).toContain('SOURCE_ADEEL_DIR=');
    expect(setupContent).toContain('INSTALL_SKILLS_DIR=');
    expect(setupContent).toContain('CODEX_ADEEL="$INSTALL_ADEEL_DIR"');
    expect(setupContent).toContain('link_codex_skill_dirs "$SOURCE_ADEEL_DIR" "$CODEX_SKILLS"');
  });
  ...
  test('Codex installs always create sidecar runtime assets for the real skill target', () => {
    ...
    expect(setupContent).toContain('create_agents_sidecar "$SOURCE_ADEEL_DIR"');
  });
  ...
  test('link_codex_skill_dirs reads from .agents/skills/', () => {
    ...
    expect(fnBody).toContain('adeel*');
  });

  test('link_claude_skill_dirs creates relative symlinks', () => {
    // Claude links should be relative: ln -snf "adeel/skill_name"
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('ln -snf "adeel/$skill_name"');
  });
  ...
  // T1: Sidecar skip guard — prevents .agents/skills/adeel from being linked as a skill
  test('link_codex_skill_dirs skips the adeel sidecar directory', () => {
    const fnStart = setupContent.indexOf('link_codex_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('done', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('[ "$skill_name" = "adeel" ] && continue');
  });

  // T2: Dynamic $ADEEL_ROOT paths in generated Codex preambles
  test('generated Codex preambles use dynamic ADEEL_ROOT paths', () => {
    const codexSkillDir = path.join(ROOT, '.agents', 'skills', 'adeel-ship');
    ...
    expect(content).toContain('ADEEL_ROOT=');
    expect(content).toContain('$ADEEL_BIN/');
  });
  ...
  test('setup supports --host kiro with install section and sed rewrites', () => {
    ...
    expect(setupContent).toContain('~/.kiro/skills/adeel');
  });
  ...
  test('create_codex_runtime_root exposes only runtime assets', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('adeel/SKILL.md');
    expect(fnBody).toContain('browse/dist');
    expect(fnBody).toContain('browse/bin');
    expect(fnBody).toContain('adeel-upgrade/SKILL.md');
    ...
    expect(fnBody).not.toContain('ln -snf "$adeel_dir" "$codex_adeel"');
  });

  test('direct Codex installs are migrated out of ~/.codex/skills/adeel', () => {
    expect(setupContent).toContain('migrate_direct_codex_install');
    expect(setupContent).toContain('$HOME/.adeel/repos/adeel');
    expect(setupContent).toContain('avoid duplicate skill discovery');
  });

  // --- Symlink prefix tests (PR #503) ---

  test('link_claude_skill_dirs applies adeel- prefix by default', () => {
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('SKILL_PREFIX');
    expect(fnBody).toContain('link_name="adeel-$skill_name"');
  });
  ...
  test('link_claude_skill_dirs preserves already-prefixed dirs', () => {
    ...
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    // adeel-* dirs should keep their name (e.g., adeel-upgrade stays adeel-upgrade)
    expect(fnBody).toContain('adeel-*) link_name="$skill_name"');
  });
  ...
  test('cleanup_old_claude_symlinks removes only adeel-pointing symlinks', () => {
    ...
    expect(fnBody).toContain('adeel-*) continue');
  });
  ...
  test('setup reads skill_prefix from config', () => {
    ...
    expect(setupContent).toContain('ADEEL_CONFIG');
  });
  ...
  test('cleanup_prefixed_claude_symlinks exists and uses readlink', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('readlink');
    expect(fnBody).toContain('adeel-$skill_name');
  });
  ...
  test('welcome message references SKILL_PREFIX', () => {
    // adeel-upgrade is always called adeel-upgrade (it's the actual dir name)
    // but the welcome section should exist near the prefix logic
    expect(setupContent).toContain('Run /adeel-upgrade anytime');
  });
});

describe('discover-skills hidden directory filtering', () => {
  test('discoverTemplates skips dot-prefixed directories', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'adeel-discover-'));
    try {
...
describe('telemetry', () => {
  test('generated SKILL.md contains telemetry start block', () => {
    ...
    expect(content).toContain('adeel-config get telemetry');
  });

  test('generated SKILL.md contains telemetry opt-in prompt', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('.telemetry-prompted');
    expect(content).toContain('Help adeel get better');
    expect(content).toContain('adeel-config set telemetry community');
    expect(content).toContain('adeel-config set telemetry anonymous');
    expect(content).toContain('adeel-config set telemetry off');
  });
...
describe('codex commands must not use inline $(git rev-parse --show-toplevel) for cwd', () => {
  ...
  // Use Bun.Glob to avoid ELOOP from .claude/skills/adeel symlink back to ROOT
  ...
  test('codex review commands must be preceded by cd "$_REPO_ROOT" (no -C support)', () => {
    ...
    for (const rel of allFiles) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (line.includes('|') && line.includes('`/adeel:codex review`')) continue;
