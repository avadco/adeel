...
// Eval result collector — accumulates test results, writes to $HOME/.adeel-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.adeel', 'test-transcripts');
...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local ${CLAUDE_PLUGIN_ROOT}/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with adeel installed globally, the fallback path
    // ${CLAUDE_PLUGIN_ROOT}/browse/dist/browse exists, so we get READY.
...
OVERRIDE: Write contributor logs to ${logsDir}/ instead of $HOME/.adeel/contributor-logs/

Now try this browse command (it will fail — there is no binary at this path):
/nonexistent/path/browse goto https://example.com

This is a adeel issue (the browse binary is missing/misconfigured).
File a contributor report about this issue. Then tell me what you filed.`,
    ...
    expect(logContent).toContain('Hey adeel team');
  ...
  testIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a adeel skill. The session preamble detected _SESSIONS=4 (the user has 4 adeel windows open).
...
describeIfSelected('QA skill E2E', ['qa-quick'], () => {
  ...
  test('/adeel:qa quick completes without browse errors', async () => {
    const result = await runSkillTest({
      prompt: `B="${browseBin}"
...
Write your report to ${qaDir}/qa-reports/qa-report.md`,
    ...
    logCost('/adeel:qa quick', result);
    recordE2E('/adeel:qa quick', 'QA skill E2E', result, {
      passed: ['success', 'error_max_turns'].includes(result.exitReason),
    });
    // browseErrors can include false positives from hallucinated paths
    if (result.browseErrors.length > 0) {
      console.warn('/adeel:qa quick browse errors (non-fatal):', result.browseErrors);
    }
...
describeIfSelected('Review skill E2E', ['review-sql-injection'], () => {
  ...
  test('/adeel:review produces findings on SQL injection branch', async () => {
    const result = await runSkillTest({
...
Run /adeel:review on the current diff (git diff main...HEAD).
Write your review findings to ${reviewDir}/review-output.md`,
    ...
    logCost('/adeel:review', result);
    recordE2E('/adeel:review SQL injection', 'Review skill E2E', result);
    expect(result.exitReason).toBe('success');
  }, 120_000);
});
...
describeIfSelected('Review enum completeness E2E', ['review-enum-completeness'], () => {
  ...
  test('/adeel:review catches missing enum handlers for new status value', async () => {
    const result = await runSkillTest({
...
The diff adds a new "returned" status to the Order model. Your job is to check if all consumers handle it.`,
    ...
    logCost('/adeel:review enum', result);
    recordE2E('/adeel:review enum completeness', 'Review enum completeness E2E', result);
    expect(result.exitReason).toBe('success');
...
describeE2E('Review design lite E2E', () => {
  ...
  test('/adeel:review catches design anti-patterns in CSS/HTML diff', async () => {
    const result = await runSkillTest({
...
Important: The design checklist should catch issues like blacklisted fonts, small font sizes, outline:none, !important, AI slop patterns (purple gradients, generic hero copy, 3-column feature grid), etc.`,
    ...
    logCost('/adeel:review design lite', result);
    recordE2E('/adeel:review design lite', 'Review design lite E2E', result);
    expect(result.exitReason).toBe('success');
...
- The report MUST exist at ${reportPath} when you finish`,
    ...
    if (!report) {
      ...
      recordE2E(`/adeel:qa ${label}`, 'Planted-bug outcome evals', result, { error: 'no report generated' });
    ...
    recordE2E(`/adeel:qa ${label}`, 'Planted-bug outcome evals', result, {
  ...
  test('/adeel:qa finds >= 2 of 5 planted bugs (static)', async () => {
  ...
  test('/adeel:qa finds >= 2 of 5 planted SPA bugs', async () => {
  ...
  test('/adeel:qa finds >= 2 of 5 planted checkout bugs', async () => {
...
`);
  ...
  test('/adeel:plan-ceo-review produces structured review output', async () => {
    const result = await runSkillTest({
...
Focus on reviewing the plan content: architecture, error handling, security, and performance.`,
    ...
    logCost('/adeel:plan-ceo-review', result);
    recordE2E('/adeel:plan-ceo-review', 'Plan CEO Review E2E', result, {
      passed: ['success', 'error_max_turns'].includes(result.exitReason),
    });
...
`);
  ...
  test('/adeel:plan-ceo-review SELECTIVE EXPANSION produces structured review output', async () => {
    const result = await runSkillTest({
      prompt: `Read plan-ceo-review/SKILL.md for the review workflow.
...
Focus on reviewing the plan content: architecture, error handling, security, and performance.`,
    ...
    logCost('/adeel:plan-ceo-review (SELECTIVE)', result);
...
`);
  ...
  test('/adeel:plan-eng-review produces structured review output', async () => {
    const result = await runSkillTest({
...
Focus on architecture, code quality, tests, and performance sections.`,
    ...
    logCost('/adeel:plan-eng-review', result);
    recordE2E('/adeel:plan-eng-review', 'Plan Eng Review E2E', result, {
      passed: ['success', 'error_max_turns'].includes(result.exitReason),
    });
    expect(['success', 'error_max_turns']).toContain(result.exitReason);
...
describeIfSelected('Retro E2E', ['retro'], () => {
  ...
  test('/adeel:retro produces analysis from git history', async () => {
    const result = await runSkillTest({
      prompt: `Read retro/SKILL.md for instructions on how to run a retrospective.

Run /adeel:retro for the last 7 days of this git repo. Skip any AskUserQuestion calls — this is non-interactive.
...
Analyze the git history and produce the narrative report as described in the SKILL.md.`,
    ...
    logCost('/adeel:retro', result);
    recordE2E('/adeel:retro', 'Retro E2E', result, {
      passed: ['success', 'error_max_turns'].includes(result.exitReason),
    });
...
describeIfSelected('QA-Only skill E2E', ['qa-only-no-fix'], () => {
  ...
  test('/adeel:qa-only produces report without using Edit tool', async () => {
    const result = await runSkillTest({
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    recordE2E('/adeel:qa-only no-fix', 'QA-Only skill E2E', result, {
    ...
    const statusLines = gitStatus.stdout.toString().trim().split('\n').filter(
      (l: string) => l.trim() && !l.includes('.prompt-tmp') && !l.includes('.adeel/') && !l.includes('qa-reports/'),
    );
...
`);
  ...
  test('/adeel:qa fix loop finds bugs and commits fixes', async () => {
...
This is a test+fix loop: find bugs, fix them in the source code, commit each fix, and re-verify.`,
    ...
    logCost('/adeel:qa fix loop', result);
    recordE2E('/adeel:qa fix loop', 'QA Fix Loop E2E', result, {
      passed: ['success', 'error_max_turns'].includes(result.exitReason),
    });
    ...
    console.log(`/adeel:qa fix loop: ${commits.length} commits total (1 initial + ${commits.length - 1} fixes)`);
...
`);
    ...
    projectDir = path.join(os.homedir(), '.adeel', 'projects', 'test-project');
  ...
  test('/adeel:plan-eng-review writes test-plan artifact to $HOME/.adeel/projects/', async () => {
...
Write your review to ${planDir}/review-output.md`,
    ...
    logCost('/adeel:plan-eng-review artifact', result);
    recordE2E('/adeel:plan-eng-review test-plan artifact', 'Plan-Eng-Review Test-Plan Artifact E2E', result, {
      passed: ['success', 'error_max_turns'].includes(result.exitReason),
    });
...
Write your findings to ${dir}/review-output.md`,
    ...
    logCost('/adeel:review base-branch', result);
    recordE2E('/adeel:review base branch detection', 'Base branch detection', result);
    expect(result.exitReason).toBe('success');
...
- The diff stat against the base branch`,
    ...
    logCost('/adeel:ship base-branch', result);
    recordE2E('/adeel:ship base branch detection', 'Base branch detection', result);
    expect(result.exitReason).toBe('success');
...
Write your retrospective to ${dir}/retro-output.md`,
    ...
    logCost('/adeel:retro base-branch', result);
    recordE2E('/adeel:retro default branch detection', 'Base branch detection', result, {
      passed: ['success', 'error_max_turns'].includes(result.exitReason),
    });
    expect(['success', 'error_max_turns']).toContain(result.exitReason);
...
describeIfSelected('Document-Release skill E2E', ['document-release'], () => {
  ...
  test('/adeel:document-release updates docs without clobbering CHANGELOG', async () => {
    const result = await runSkillTest({
      prompt: `Read the file document-release/SKILL.md for the document-release workflow instructions.

Run the /adeel:document-release workflow on this repo. The base branch is "main".
...
- After editing, just commit the changes locally.`,
    ...
    logCost('/adeel:document-release', result);
    ...
    const exitOk = ['success', 'error_max_turns'].includes(result.exitReason);
    recordE2E('/adeel:document-release', 'Document-Release skill E2E', result, {
...
describeE2E('Deferred skill E2E', () => {
  ...
  test.todo('/adeel:ship completes full workflow');

  // Setup-browser-cookies requires interactive browser picker UI
  test.todo('/adeel:setup-browser-cookies imports cookies');

});

// --- adeel-upgrade E2E ---

describeIfSelected('adeel-upgrade E2E', ['adeel-upgrade-happy-path'], () => {
  let upgradeDir: string;
  let remoteDir: string;

  beforeAll(() => {
    upgradeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'skill-e2e-upgrade-'));
    remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'adeel-remote-'));
    ...
    // Create mock adeel install directory (local-git type)
    const mockAdeel = path.join(upgradeDir, '.claude', 'skills', 'adeel');
    fs.mkdirSync(mockAdeel, { recursive: true });

    // Init as a git repo
    run('git', ['init'], mockAdeel);
    run('git', ['config', 'user.email', 'test@test.com'], mockAdeel);
    run('git', ['config', 'user.name', 'Test'], mockAdeel);

    // Create bare remote
    run('git', ['init', '--bare'], remoteDir);
    run('git', ['remote', 'add', 'origin', remoteDir], mockAdeel);

    // Write old version files
    fs.writeFileSync(path.join(mockAdeel, 'VERSION'), '0.5.0\n');
    fs.writeFileSync(path.join(mockAdeel, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    fs.writeFileSync(path.join(mockAdeel, 'setup'),
      '#!/bin/bash\necho "Setup completed"\n', { mode: 0o755 });

    // Initial commit + push
    run('git', ['add', '.'], mockAdeel);
    run('git', ['commit', '-m', 'initial'], mockAdeel);
    run('git', ['push', '-u', 'origin', 'HEAD:main'], mockAdeel);

    // Create new version (simulate upstream release)
    fs.writeFileSync(path.join(mockAdeel, 'VERSION'), '0.6.0\n');
    fs.writeFileSync(path.join(mockAdeel, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.6.0 — 2026-03-15\n\n- New feature: interactive design review\n- Fix: snapshot flag validation\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    run('git', ['add', '.'], mockAdeel);
    run('git', ['commit', '-m', 'release 0.6.0'], mockAdeel);
    run('git', ['push', 'origin', 'HEAD:main'], mockAdeel);

    // Reset working copy back to old version
    run('git', ['reset', '--hard', 'HEAD~1'], mockAdeel);

    // Copy adeel-upgrade skill
    fs.mkdirSync(path.join(upgradeDir, 'adeel-upgrade'), { recursive: true });
    fs.copyFileSync(
      path.join(ROOT, 'adeel-upgrade', 'SKILL.md'),
      path.join(upgradeDir, 'adeel-upgrade', 'SKILL.md'),
    );
  ...
  testIfSelected('adeel-upgrade-happy-path', async () => {
    const mockAdeel = path.join(upgradeDir, '.claude', 'skills', 'adeel');
    const result = await runSkillTest({
      prompt: `Read adeel-upgrade/SKILL.md for the upgrade workflow.

You are running /adeel-upgrade standalone. The adeel installation is at ./.claude/skills/adeel (local-git type — it has a .git directory with an origin remote).
...
IMPORTANT: The install directory is at ./.claude/skills/adeel — use that exact path.`,
      workingDirectory: upgradeDir,
      maxTurns: 20,
      timeout: 180_000,
      testName: 'adeel-upgrade-happy-path',
      runId,
    });

    logCost('/adeel-upgrade happy path', result);

    // Check that the version was updated
    const versionAfter = fs.readFileSync(path.join(mockAdeel, 'VERSION'), 'utf-8').trim();
    const output = result.output || '';
    ...
    recordE2E('/adeel-upgrade happy path', 'adeel-upgrade E2E', result, {
...
Write DESIGN.md and CLAUDE.md (or update it) in the working directory.`,
    ...
    logCost('/adeel:design-consultation core', result);
    ...
    recordE2E('/adeel:design-consultation core', 'Design Consultation E2E', result, {
...
Write DESIGN.md to the working directory.`,
    ...
    logCost('/adeel:design-consultation research', result);
    ...
    recordE2E('/adeel:design-consultation research', 'Design Consultation E2E', result, {
...
Skip research. Skip font preview. Skip any AskUserQuestion calls — this is non-interactive.`,
    ...
    logCost('/adeel:design-consultation existing', result);
    ...
    recordE2E('/adeel:design-consultation existing', 'Design Consultation E2E', result, {
...
Skip research. Skip any AskUserQuestion calls — this is non-interactive. Generate the font and color preview page but write it to ./design-preview.html instead of /tmp/ (do NOT run the open command). Then write DESIGN.md.`,
    ...
    logCost('/adeel:design-consultation preview', result);
    ...
    recordE2E('/adeel:design-consultation preview', 'Design Consultation E2E', result, {
...
IMPORTANT: Do NOT try to browse any URLs or use a browse binary. This is a plan review, not a live site audit. Just read the plan file, review it, and edit it to fix the gaps.`,
    ...
    logCost('/adeel:plan-design-review plan-mode', result);
    ...
    recordE2E('/adeel:plan-design-review plan-mode', 'Plan Design Review E2E', result, {
...
IMPORTANT: Do NOT try to browse any URLs or use a browse binary. This is a plan review, not a live site audit.`,
    ...
    logCost('/adeel:plan-design-review no-ui-scope', result);
    ...
    recordE2E('/adeel:plan-design-review no-ui-scope', 'Plan Design Review E2E', result, {
...
`);
  ...
  test('Test 7: /adeel:design-review audits and fixes design issues', async () => {
...
Review the site at ${serverUrl}. Use --quick mode. Skip any AskUserQuestion calls — this is non-interactive. Fix up to 3 issues max. Write your report to ./design-audit.md.`,
    ...
    logCost('/adeel:design-review fix', result);
    ...
    recordE2E('/adeel:design-review fix', 'Design Review E2E', result, {
      passed: ['success', 'error_max_turns'].includes(result.exitReason),
    });
...
`);
  ...
  test('/adeel:qa bootstrap + regression test on zero-test project', async () => {
...
This is a test+fix loop: find bugs, fix them, write regression tests, commit each fix.`,
    ...
    logCost('/adeel:qa bootstrap', result);
    recordE2E('/adeel:qa bootstrap + regression test', 'Test Bootstrap E2E', result, {
      passed: ['success', 'error_max_turns'].includes(result.exitReason),
    });
...
describeIfSelected('Test Coverage Audit E2E', ['ship-coverage-audit'], () => {
  ...
  test('/adeel:ship Step 3.4 produces coverage diagram', async () => {
    const result = await runSkillTest({
...
Output the diagram directly.`,
    ...
    logCost('/adeel:ship coverage audit', result);
    recordE2E('/adeel:ship Step 3.4 coverage audit', 'Test Coverage Audit E2E', result, {
      passed: result.exitReason === 'success',
    });
...
describeIfSelected('Review Coverage Audit E2E', ['review-coverage-audit'], () => {
  ...
  test('/adeel:review Step 4.75 produces coverage diagram', async () => {
    const result = await runSkillTest({
...
Output the diagram directly.`,
    ...
    logCost('/adeel:review coverage audit', result);
    recordE2E('/adeel:review Step 4.75 coverage audit', 'Review Coverage Audit E2E', result, {
      passed: result.exitReason === 'success',
    });
...
describeIfSelected('Plan Eng Review Coverage Audit E2E', ['plan-eng-coverage-audit'], () => {
  ...
  test('/adeel:plan-eng-review coverage audit traces plan codepaths', async () => {
    const result = await runSkillTest({
...
Output the diagram directly.`,
    ...
    logCost('/adeel:plan-eng-review coverage audit', result);
    recordE2E('/adeel:plan-eng-review coverage audit', 'Plan Eng Review Coverage Audit E2E', result, {
      passed: result.exitReason === 'success',
    });
...
`);
  ...
  test('/adeel:ship triage correctly classifies in-branch vs pre-existing failures', async () => {
    const result = await runSkillTest({
      prompt: `Read the file ship/SKILL.md for the ship workflow instructions.
...
This is a solo repo (REPO_MODE=solo). For pre-existing failures, recommend fixing now.`,
    ...
    logCost('/adeel:ship triage', result);
    ...
    recordE2E('/adeel:ship triage', 'Test Failure Triage E2E', result, {
...
describeIfSelected('Codex skill E2E', ['codex-review'], () => {
  ...
  test('/adeel:codex review produces findings and GATE verdict', async () => {
...
Read codex-SKILL.md for the /adeel:codex skill instructions.
Run /adeel:codex review to review the current diff against main.
Write the full output (including the GATE verdict) to ${codexDir}/codex-output.md`,
    ...
    logCost('/adeel:codex review', result);
    recordE2E('/adeel:codex review', 'Codex skill E2E', result);
    expect(result.exitReason).toBe('success');
...
describeIfSelected('Office Hours Spec Review E2E', ['office-hours-spec-review'], () => {
  ...
  test('/adeel:office-hours SKILL.md contains spec review loop', async () => {
    const result = await runSkillTest({
...
Write your summary to ${ohDir}/spec-review-summary.md`,
    ...
    logCost('/adeel:office-hours spec review', result);
...
describeIfSelected('Plan CEO Review Benefits-From E2E', ['plan-ceo-review-benefits'], () => {
  ...
  test('/adeel:plan-ceo-review SKILL.md contains prerequisite skill offer', async () => {
    const result = await runSkillTest({
...
1. Is /adeel:office-hours offered as a prerequisite?
...
Write your summary to ${benefitsDir}/benefits-summary.md`,
    ...
    logCost('/adeel:plan-ceo-review benefits-from', result);
