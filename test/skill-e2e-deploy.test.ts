...
describeIfSelected('Land-and-Deploy skill E2E', ['land-and-deploy-workflow'], () => {
  ...
  testConcurrentIfSelected('land-and-deploy-workflow', async () => {
    const result = await runSkillTest({
      prompt: `Read land-and-deploy/SKILL.md for the /adeel:land-and-deploy skill instructions.
...
5. Write a deploy report skeleton to .adeel/deploy-reports/report.md showing the
...
Do NOT use AskUserQuestion. Do NOT run gh or fly commands.`,
    ...
    logCost('/adeel:land-and-deploy', result);
    recordE2E(evalCollector, '/adeel:land-and-deploy workflow', 'Land-and-Deploy skill E2E', result);
    ...
    const reportDir = path.join(landDir, '.adeel', 'deploy-reports');
...
You are on branch feat/first-deploy. This is the FIRST TIME running /adeel:land-and-deploy
...
5. Write the dry-run report to .adeel/deploy-reports/dry-run-validation.md
...
Just demonstrate the first-run dry-run output.`,
    ...
    logCost('/adeel:land-and-deploy first-run', result);
    recordE2E(evalCollector, '/adeel:land-and-deploy first-run', 'Land-and-Deploy first-run E2E', result);
    expect(result.exitReason).toBe('success');

    // Verify dry-run report was created
    const reportDir = path.join(firstRunDir, '.adeel', 'deploy-reports');
    expect(fs.existsSync(reportDir)).toBe(true);
...
(adeel-review-read would return NO_REVIEWS).

Simulate what the readiness gate would show:
1. Run adeel-review-read equivalent (simulate NO_REVIEWS output)
2. Determine review staleness: Eng Review should be "NOT RUN"
3. Note that Step 3.5a-bis would offer an inline review
4. Write a simulated readiness report to .adeel/deploy-reports/readiness-report.md
...
Show what the readiness gate output would look like.`,
    ...
    logCost('/adeel:land-and-deploy review-gate', result);
    recordE2E(evalCollector, '/adeel:land-and-deploy review-gate', 'Land-and-Deploy review gate E2E', result);
    expect(result.exitReason).toBe('success');

    // Verify readiness report was created
    const reportDir = path.join(reviewDir, '.adeel', 'deploy-reports');
    expect(fs.existsSync(reportDir)).toBe(true);
...
describeIfSelected('Canary skill E2E', ['canary-workflow'], () => {
  ...
  testConcurrentIfSelected('canary-workflow', async () => {
    const result = await runSkillTest({
      prompt: `Read canary/SKILL.md for the /adeel:canary skill instructions.
...
1. Create the .adeel/canary-reports/ directory structure
2. Write a simulated baseline.json to .adeel/canary-reports/baseline.json with the
...
3. Write a simulated canary report to .adeel/canary-reports/canary-report.md following
...
Just create the directory structure and report files showing the correct schema.`,
    ...
    logCost('/adeel:canary', result);
    recordE2E(evalCollector, '/adeel:canary workflow', 'Canary skill E2E', result);
    expect(result.exitReason).toBe('success');

    expect(fs.existsSync(path.join(canaryDir, '.adeel', 'canary-reports'))).toBe(true);
    const reportDir = path.join(canaryDir, '.adeel', 'canary-reports');
...
describeIfSelected('Benchmark skill E2E', ['benchmark-workflow'], () => {
  ...
  testConcurrentIfSelected('benchmark-workflow', async () => {
    const result = await runSkillTest({
      prompt: `Read benchmark/SKILL.md for the /adeel:benchmark skill instructions.

You are simulating a benchmark run. There is NO browse daemon available and NO production URL.

Instead, demonstrate you understand the workflow:
1. Create the .adeel/benchmark-reports/ directory structure including baselines/
2. Write a simulated baseline.json to .adeel/benchmark-reports/baselines/baseline.json
...
3. Write a simulated benchmark report to .adeel/benchmark-reports/benchmark-report.md
...
Just create the files showing the correct schema and report format.`,
    ...
    logCost('/adeel:benchmark', result);
    recordE2E(evalCollector, '/adeel:benchmark workflow', 'Benchmark skill E2E', result);
    expect(result.exitReason).toBe('success');

    expect(fs.existsSync(path.join(benchDir, '.adeel', 'benchmark-reports'))).toBe(true);
    const baselineDir = path.join(benchDir, '.adeel', 'benchmark-reports', 'baselines');
...
describeIfSelected('Setup-Deploy skill E2E', ['setup-deploy-workflow'], () => {
  ...
  testConcurrentIfSelected('setup-deploy-workflow', async () => {
    const result = await runSkillTest({
      prompt: `Read setup-deploy/SKILL.md for the /adeel:setup-deploy skill instructions.

This repo has a fly.toml with app = "my-cool-app". Run the /adeel:setup-deploy workflow:
...
Just detect the platform and write the config.`,
    ...
    logCost('/adeel:setup-deploy', result);
    recordE2E(evalCollector, '/adeel:setup-deploy workflow', 'Setup-Deploy skill E2E', result);
    expect(result.exitReason).toBe('success');
