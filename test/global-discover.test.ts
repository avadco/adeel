...
const scriptPath = join(import.meta.dir, "..", "bin", "adeel-global-discover.ts");

describe("adeel-global-discover", () => {
  describe("normalizeRemoteUrl", () => {
    // Dynamically import to test the exported function
    let normalizeRemoteUrl: (url: string) => string;

    beforeEach(async () => {
      const mod = await import("../bin/adeel-global-discover.ts");
    ...
    test("SSH and HTTPS for same repo normalize to same URL", () => {
      const ssh = normalizeRemoteUrl("git@github.com:avadco/adeel.git");
      const https = normalizeRemoteUrl("https://github.com/avadco/adeel.git");
      const httpsNoDotGit = normalizeRemoteUrl("https://github.com/avadco/adeel");
