 ...
 * $HOME/.adeel/projects/$SLUG/evals/{version}-{branch}-{tier}-{timestamp}.json,
...
const LEGACY_EVAL_DIR = path.join(os.homedir(), '.adeel-dev', 'evals');

/**
 * Detect project-scoped eval dir via adeel-slug.
 * Falls back to legacy $HOME/.adeel-dev/evals/ if slug detection fails.
 */
export function getProjectEvalDir(): string {
  try {
    // Try repo-local adeel-slug first, then global install
    const localSlug = spawnSync('bash', ['-c', '${CLAUDE_PLUGIN_ROOT}/bin/adeel-slug 2>/dev/null || ${CLAUDE_PLUGIN_ROOT}/bin/adeel-slug 2>/dev/null'], {
    ...
    if (output) {
      ...
      if (slugMatch && slugMatch[1]) {
        const dir = path.join(os.homedir(), '.adeel', 'projects', slugMatch[1], 'evals');
