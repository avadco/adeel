...
describe('session-runner observability', () => {
  test('1: sanitizeTestName strips slashes and leading dashes', () => {
    expect(sanitizeTestName('/adeel:plan-ceo-review')).toBe('plan-ceo-review');
  ...
  test('2: heartbeat file path uses $HOME/.adeel-dev/e2e-live.json', () => {
    // Just verify the constant is correct — actual write is tested by E2E
    const expected = path.join(os.homedir(), '.adeel-dev', 'e2e-live.json');
  ...
  test('9: renderDashboard shows completed tests and current test', () => {
    ...
    const partial: PartialData = {
      tests: [
        ...
        { name: '/adeel:review', passed: true, cost_usd: 0.17, duration_ms: 63000, turns_used: 13 },
      ],
      total_cost_usd: 0.24,
      _partial: true,
    };
    ...
    expect(output).toContain('/adeel:review');
