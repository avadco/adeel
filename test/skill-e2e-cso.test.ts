...
`);
  ...
  test('/adeel:cso finds planted vulnerabilities', async () => {
    const result = await runSkillTest({
      prompt: `Read the file ${path.join(ROOT, 'cso', 'SKILL.md')} for the CSO skill instructions.

Run /adeel:cso on this repo (full daily audit, no flags).
...
- Save the report to .adeel/security-reports/.`,
    ...
    const reportDir = path.join(csoDir, '.adeel', 'security-reports');
...
`);
  ...
  test('/adeel:cso --diff scopes to branch changes', async () => {
    const result = await runSkillTest({
      prompt: `Read the file ${path.join(ROOT, 'cso', 'SKILL.md')} for the CSO skill instructions.

Run /adeel:cso --diff on this repo. The base branch is "main".
...
`);
  ...
  test('/adeel:cso --infra runs infrastructure phases only', async () => {
    const result = await runSkillTest({
      prompt: `Read the file ${path.join(ROOT, 'cso', 'SKILL.md')} for the CSO skill instructions.

Run /adeel:cso --infra on this repo. This should run infrastructure-only phases (0-6, 12-14).
...
- Skip the preamble (adeel-update-check, telemetry, etc.) — go straight to the audit.
