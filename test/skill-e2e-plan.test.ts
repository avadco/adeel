...
Focus on reviewing the plan content: architecture, error handling, security, and performance.`,
    ...
    recordE2E(evalCollector, '/adeel:plan-ceo-review', 'Plan CEO Review E2E', result, {
...
Focus on architecture, code quality, tests, and performance sections.`,
    ...
    recordE2E(evalCollector, '/adeel:plan-eng-review', 'Plan Eng Review E2E', result, {
      passed: ['success', 'error_max_turns'].includes(result.exitReason),
    });
    expect(['success', 'error_max_turns']).toContain(result.exitReason);
...
Write your review to ${planDir}/review-output.md`,
    ...
    recordE2E(evalCollector, '/adeel:plan-eng-review test-plan artifact', 'Plan-Eng-Review Test-Plan Artifact E2E', result, {
      passed: ['success', 'error_max_turns'].includes(result.exitReason),
    });
...
// Verifies that plan-eng-review writes a "## ADEEL REVIEW REPORT" section
...
`);
  ...
  test('/adeel:plan-eng-review writes ADEEL REVIEW REPORT to plan file', async () => {
    const result = await runSkillTest({
      prompt: `Read plan-eng-review/SKILL.md for the review workflow.
...
CRITICAL REQUIREMENT: plan.md IS the plan file for this review session. After completing your review, you MUST write a "## ADEEL REVIEW REPORT" section to the END of plan.md, exactly as described in the "Plan File Review Report" section of SKILL.md. If adeel-review-read is not available or returns NO_REVIEWS, write the placeholder table with all four review rows (CEO, Codex, Eng, Design). Use the Edit tool to append to plan.md — do NOT overwrite the existing plan content.
...
This review report at the bottom of the plan is the MOST IMPORTANT deliverable of this test.`,
    ...
    logCost('/adeel:plan-eng-review report', result);
    ...
    expect(planContent).toContain('## ADEEL REVIEW REPORT');

    // Report should be at the bottom of the file
    const reportIndex = planContent.lastIndexOf('## ADEEL REVIEW REPORT');
