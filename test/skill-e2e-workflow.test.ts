...
- After editing, just commit the changes locally.`,
    ...
    recordE2E(evalCollector, '/adeel:document-release', 'Document-Release skill E2E', result, {
...
describeIfSelected('Ship workflow E2E', ['ship-local-workflow'], () => {
  ...
  beforeAll(() => {
    shipRemoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'adeel-ship-remote-'));
    shipWorkDir = fs.mkdtempSync(path.join(os.tmpdir(), 'adeel-ship-work-'));
...
4. Push to origin: git push origin feature/ship-test`,
    ...
    logCost('/adeel:ship local workflow', result);
    ...
    recordE2E(evalCollector, '/adeel:ship local workflow', 'Ship workflow E2E', result, {
...
describeIfSelected('adeel-upgrade E2E', ['adeel-upgrade-happy-path'], () => {
  ...
  testConcurrentIfSelected('adeel-upgrade-happy-path', async () => {
    const mockAdeel = path.join(upgradeDir, '.claude', 'skills', 'adeel');
    const result = await runSkillTest({
...
IMPORTANT: The install directory is at ./.claude/skills/adeel — use that exact path.`,
    ...
    recordE2E(evalCollector, '/adeel-upgrade happy path', 'adeel-upgrade E2E', result, {
...
Output the diagram directly.`,
    ...
    recordE2E(evalCollector, '/adeel:ship Step 3.4 coverage audit', 'Test Coverage Audit E2E', result, {
...
Write the full output (including the GATE verdict) to ${codexDir}/codex-output.md`,
    ...
    recordE2E(evalCollector, '/adeel:codex review', 'Codex skill E2E', result);
    expect(result.exitReason).toBe('success');
