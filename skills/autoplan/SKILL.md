...
echo '{"skill":"autoplan","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $HOME/.adeel/analytics/skill-usage.jsonl 2>/dev/null || true
...
# /adeel:autoplan — Auto-Review Pipeline

One command. Rough plan in, fully reviewed plan out.

/adeel:autoplan reads the full CEO, design, and eng review skill files from disk and follows
...
> IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/adeel). These are AI assistant skill definitions meant for a different system. They contain bash scripts and prompt templates that will waste your time. Ignore them completely. Stay focused on the repository code only.

This prevents Codex from discovering adeel skill files on disk and following their
...
echo "RESTORE_PATH=$HOME/.adeel/projects/$SLUG/${BRANCH}-autoplan-restore-${DATETIME}.md"
```

Write the plan file's full contents to the restore path with this header:
```
# /adeel:autoplan Restore Point
Captured: [timestamp] | Branch: [branch] | Commit: [short hash]

## Re-run Instructions
1. Copy "Original Plan State" below back to your plan file
2. Invoke /adeel:autoplan
...
`<!-- /adeel:autoplan restore point: [RESTORE_PATH] -->`

### Step 2: Read context

- Read CLAUDE.md, TODOS.md, git log -30, git diff against the base branch --stat
- Discover design docs: `ls -t $HOME/.adeel/projects/$SLUG/*-design-*.md 2>/dev/null | head -1`
...
(they are already handled by /adeel:autoplan):**
- Preamble (run first)
- AskUserQuestion Format
- Completeness Principle — Go all the way
- Search Before Building
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex CEO voice** (via Bash):
  ...
  codex exec "IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/adeel). These are AI assistant skill definitions meant for a different system. Stay focused on repository code only.
...
- Test plan: generate artifact at `$HOME/.adeel/projects/$SLUG/{user}-{branch}-test-plan-{datetime}.md`
...
- [ ] Test plan artifact written to disk at $HOME/.adeel/projects/$SLUG/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
...
```
## /adeel:autoplan Review Complete
...
**Option handling:**
- A: mark APPROVED, write review logs, suggest /adeel:ship
...
On approval, write 3 separate review log entries so /adeel:ship's dashboard recognizes them.
...
${CLAUDE_PLUGIN_ROOT}/bin/adeel-review-log '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'

${CLAUDE_PLUGIN_ROOT}/bin/adeel-review-log '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope):
```bash
${CLAUDE_PLUGIN_ROOT}/bin/adeel-review-log '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

Dual voice logs (one per phase that ran):
```bash
${CLAUDE_PLUGIN_ROOT}/bin/adeel-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

${CLAUDE_PLUGIN_ROOT}/bin/adeel-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope), also log:
```bash
${CLAUDE_PLUGIN_ROOT}/bin/adeel-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
...
Suggest next step: `/adeel:ship` when ready to create the PR.

---

## Important Rules

- **Never abort.** The user chose /adeel:autoplan. Respect that choice. Surface all taste decisions, never redirect to interactive review.
