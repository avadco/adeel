/**
 * adeel browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.adeel/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.adeel/browse-{console,network,dialog}.log
...
function generateHelpText(): string {
  ...
  const lines = ['adeel browse — headless browser for AI agents', '', 'Commands:'];
...
const SESSIONS_DIR = path.join(process.env.HOME || '/tmp', '.adeel', 'sidebar-sessions');
...
function findBrowseBin(): string {
  const candidates = [
    ...
    path.resolve(__dirname, '..', '..', '.claude', 'skills', 'adeel', 'browse', 'dist', 'browse'),
    path.join(process.env.HOME || '', '.claude', 'skills', 'adeel', 'browse', 'dist', 'browse'),
  ];
...
function shortenPath(str: string): string {
  return str
    ...
    .replace(/\.claude\/skills\/adeel\//g, '')
    .replace(/adeel:browse\/dist\/browse/g, '$B');
}
...
function createWorktree(sessionId: string): string | null {
  try {
    ...
    const worktreeDir = path.join(process.env.HOME || '/tmp', '.adeel', 'worktrees', sessionId.slice(0, 8));
...
function spawnClaude(userMessage: string, extensionUrl?: string | null): void {
  ...
  const agentQueue = process.env.SIDEBAR_QUEUE_PATH || path.join(process.env.HOME || '/tmp', '.adeel', 'sidebar-agent-queue.jsonl');
  const adeelDir = path.dirname(agentQueue);
  ...
  try {
    fs.mkdirSync(adeelDir, { recursive: true });
...
async function shutdown() {
  ...
  const profileDir = path.join(process.env.HOME || '/tmp', '.adeel', 'chromium-profile');
