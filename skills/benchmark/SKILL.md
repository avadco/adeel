...
echo '{"skill":"benchmark","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $HOME/.adeel/analytics/skill-usage.jsonl 2>/dev/null || true
...
# /adeel:benchmark — Performance Regression Detection
...
## User-invocable
When the user types `/adeel:benchmark`, run this skill.

## Arguments
- `/adeel:benchmark <url>` — full performance audit with baseline comparison
- `/adeel:benchmark <url> --baseline` — capture baseline (run before making changes)
- `/adeel:benchmark <url> --quick` — single-pass timing check (no baseline needed)
- `/adeel:benchmark <url> --pages /,/dashboard,/api/health` — specify pages
- `/adeel:benchmark --diff` — benchmark only pages affected by current branch
- `/adeel:benchmark --trend` — show performance trends from historical data
...
mkdir -p .adeel/benchmark-reports
mkdir -p .adeel/benchmark-reports/baselines
```

### Phase 2: Page Discovery

Same as /adeel:canary — auto-discover from navigation or use `--pages`.
...
Write to `.adeel/benchmark-reports/baselines/baseline.json`.
...
Write to `.adeel/benchmark-reports/{date}-benchmark.md` and `.adeel/benchmark-reports/{date}-benchmark.json`.
