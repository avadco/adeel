...
description: |
  ...
  verifies production health via canary checks. Takes over after /adeel:ship
...
echo '{"skill":"land-and-deploy","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $HOME/.adeel/analytics/skill-usage.jsonl 2>/dev/null || true
...
**If the platform detected above is GitLab or unknown:** STOP with: "GitLab support for /adeel:land-and-deploy is not yet implemented. Run `/adeel:ship` to create the MR, then merge manually via the GitLab web UI." Do not proceed.

# /adeel:land-and-deploy — Merge, Deploy, Verify

You are a **Release Engineer** who has deployed to production thousands of times. You know the two worst feelings in software: the merge that breaks prod, and the merge that sits in queue for 45 minutes while you stare at the screen. Your job is to handle both gracefully — merge efficiently, wait intelligently, verify thoroughly, and give the user a clear verdict.

This skill picks up where `/adeel:ship` left off. `/adeel:ship` creates the PR. You merge it, wait for deploy, and verify production.

## User-invocable
When the user types `/adeel:land-and-deploy`, run this skill.

## Arguments
- `/adeel:land-and-deploy` — auto-detect PR from current branch, no post-deploy URL
- `/adeel:land-and-deploy <url>` — auto-detect PR, verify deploy at this URL
- `/adeel:land-and-deploy #123` — specific PR number
- `/adeel:land-and-deploy #123 <url>` — specific PR + verification URL

## Non-interactive philosophy (like /adeel:ship) — with one critical gate

This is a **mostly automated** workflow. Do NOT ask for confirmation at any step except
the ones listed below. The user said `/adeel:land-and-deploy` which means DO IT — but verify
readiness first.
...
gh auth status
```
If not authenticated, **STOP**: "I need GitHub CLI access to merge your PR. Run `gh auth login` to connect, then try `/adeel:land-and-deploy` again."
...
5. Validate the PR state:
   - If no PR exists: **STOP.** "No PR found for this branch. Run `/adeel:ship` first to create a PR, then come back here to land and deploy it."
   - If `state` is `MERGED`: "This PR is already merged — nothing to deploy. If you need to verify the deploy, run `/adeel:canary <url>` instead."
...
Check whether this project has been through a successful `/adeel:land-and-deploy` before,
and whether the deploy configuration has changed since then:

```bash
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/adeel-slug 2>/dev/null)"
if [ ! -f $HOME/.adeel/projects/$SLUG/land-deploy-confirmed ]; then
  echo "FIRST_RUN"
else
  # Check if deploy config has changed since confirmation
  SAVED_HASH=$(cat $HOME/.adeel/projects/$SLUG/land-deploy-confirmed 2>/dev/null)
...
**If FIRST_RUN:** This is the first time `/adeel:land-and-deploy` is running for this project. Before doing anything irreversible, show the user exactly what will happen. This is a dry run — explain, validate, and confirm.
...
If you want to persist deploy settings for future runs, suggest the user run `/adeel:setup-deploy`.
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/adeel-review-read 2>/dev/null
```
...
- **RECOMMENDATION:** Choose A if all validations passed. Choose B if there are issues to fix. Choose C to run /adeel:setup-deploy for a more thorough configuration.
...
- C) I want to configure this more carefully first (runs /adeel:setup-deploy) (Completeness: 10/10)

**If A:** Tell the user: "Great — I've saved this configuration. Next time you run `/adeel:land-and-deploy`, I'll skip the dry run and go straight to readiness checks. If your deploy setup changes (new platform, different workflows, updated URLs), I'll automatically re-run the dry run to make sure I still have it right."

Save the deploy config fingerprint so we can detect future changes:
```bash
mkdir -p $HOME/.adeel/projects/$SLUG
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>/dev/null | xargs cat 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > $HOME/.adeel/projects/$SLUG/land-deploy-confirmed
```
Continue to Step 2.

**If B:** **STOP.** "Tell me what's different about your setup and I'll adjust. You can also run `/adeel:setup-deploy` to walk through the full configuration."

**If C:** **STOP.** "Running `/adeel:setup-deploy` will walk through your deploy platform, production URL, and health checks in detail. It saves everything to CLAUDE.md so I'll know exactly what to do next time. Run `/adeel:land-and-deploy` again when that's done."
...
```
If `CONFLICTING`: **STOP.** "This PR has merge conflicts with the base branch. Resolve the conflicts and push, then run `/adeel:land-and-deploy` again."
...
- B) Stop and run a full `/adeel:review` first — deeper analysis, more thorough (Completeness: 10/10)
...
```
Apply each checklist item to the current diff. This is the same quick review that `/adeel:ship`
...
and tell the user: "I found and fixed a few issues during the review. The fixes are committed — run `/adeel:land-and-deploy` again to pick them up and continue where we left off."

**If no issues found:** Tell the user: "Review checklist passed — no issues found in the diff."

**If B:** **STOP.** "Good call — run `/adeel:review` for a thorough pre-landing review. When that's done, run `/adeel:land-and-deploy` again and I'll pick up right where we left off."
...
ls -t $HOME/.adeel-dev/evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -20
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t $HOME/.adeel-dev/evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -5
```
...
new features (new files, new commands, new skills): **WARNING — /adeel:document-release
...
- If reviews are stale: "Run `/adeel:review` or `/adeel:autoplan` to review the current code, then `/adeel:land-and-deploy` again."
- If E2E not run: "Run your E2E tests to make sure nothing is broken, then come back."
- If docs not updated: "Run `/adeel:document-release` to update CHANGELOG and docs."
...
Then run `adeel-diff-scope` to classify the changes:

```bash
eval $(${CLAUDE_PLUGIN_ROOT}/bin/adeel-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
Then tell the user: "Staging looks good. When you're ready for production, run `/adeel:land-and-deploy` again."
**STOP.** The user can re-run `/adeel:land-and-deploy` later for production.
...
```bash
$B snapshot -i -a -o ".adeel/deploy-reports/post-deploy.png"
```
...
Save report to `.adeel/deploy-reports/{date}-pr{number}-deploy.md`.
...
- If a production URL was verified: "Want extended monitoring? Run `/adeel:canary <url>` to watch the site for the next 10 minutes."
- If performance data was collected: "Want a deeper performance analysis? Run `/adeel:benchmark <url>`."
- "Need to update docs? Run `/adeel:document-release` to sync README, CHANGELOG, and other docs with what you just shipped."
...
- **Single-pass verification, not continuous monitoring.** `/adeel:land-and-deploy` checks once. `/adeel:canary` does the extended monitoring loop.
