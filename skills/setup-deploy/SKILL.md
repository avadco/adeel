...
version: 1.0.0
description: |
  Configure deployment settings for /adeel:land-and-deploy. Detects your deploy
  platform (Fly.io, Render, Vercel, Netlify, Heroku, GitHub Actions, custom),
  production URL, health check endpoints, and deploy status commands. Writes
  the configuration to CLAUDE.md so all future deploys are automatic.
  Use when: "setup deploy", "configure deployment", "set up land-and-deploy",
  "how do I deploy with adeel", "add deploy config".
allowed-tools:
  - Bash
  - Read
  - Write
...
# /adeel:setup-deploy — Configure Deployment for adeel
...
merge to main, Render picks it up automatically. The "deploy wait" in /adeel:land-and-deploy
...
3. **How can adeel check if a deploy succeeded?**
...
- **Idempotent.** Running /adeel:setup-deploy multiple times overwrites the previous config cleanly.
