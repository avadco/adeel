...
description: |
  ...
  visual audits, use /adeel:design-review. Use when asked to "review the design plan"
  or "design critique".
...
echo '{"skill":"plan-design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $HOME/.adeel/analytics/skill-usage.jsonl 2>/dev/null || true
...
### The adeel designer — YOUR PRIMARY TOOL

You have the **adeel designer**, an AI mockup generator that creates real visual mockups
...
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/${CLAUDE_PLUGIN_ROOT}/design/dist/design" ] && D="$_ROOT/${CLAUDE_PLUGIN_ROOT}/design/dist/design"
[ -z "$D" ] && D=${CLAUDE_PLUGIN_ROOT}/design/dist/design
if [ -x "$D" ]; then
  echo "DESIGN_READY: $D"
else
...
- If no DESIGN.md: "No design system found. Recommend running /adeel:design-consultation first. Proceeding with universal design principles."
...
adeel designer is available (`DESIGN_READY` was printed during setup), **generate
mockups immediately.** Do not ask permission. This is the default behavior.

Tell the user: "Generating visual mockups with the adeel designer. This is how we
...
`$HOME/.adeel/projects/$SLUG/designs/` (user config directory, not project files).
Mockups are design artifacts that inform the plan, not code changes. The adeel
...
- `mkdir -p $HOME/.adeel/projects/$SLUG/designs/...`
...
fewer variants and benefits from sequential control. Note: /adeel:design-shotgun uses
...
**If `DESIGN_NOT_AVAILABLE`:** Tell the user: "The adeel designer isn't set up yet. Run `$D setup` to enable visual mockups. Proceeding with text-only review, but you're missing the best part." Then proceed to review passes with text-based review.
...
**Log the result:**
```bash
${CLAUDE_PLUGIN_ROOT}/bin/adeel-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
Re-run loop: invoke /adeel:plan-design-review again → re-rate → sections at 8+ get a quick pass, sections below 8 get full treatment.
...
```bash
$D generate --brief "<description of what 10/10 looks like for this dimension>" --output /tmp/adeel-ideal-<dimension>.png
```
...
Source: [OpenAI "Designing Delightful Frontends with GPT-5.4"](https://developers.openai.com/blog/designing-delightful-frontends-with-gpt-5-4) (Mar 2026) + adeel design methodology.
...
FIX TO 10: If DESIGN.md exists, annotate with specific tokens/components. If no DESIGN.md, flag the gap and recommend `/adeel:design-consultation`.
...
If all passes 8+: "Plan is design-complete. Run /adeel:design-review after implementation for visual QA."
...
|----------------|-------------|-----------|-------|
| [screen name]  | $HOME/.adeel/projects/$SLUG/designs/[folder]/[filename].png | [brief description] | [constraints from review] |
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/adeel-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
