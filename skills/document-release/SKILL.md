...
echo '{"skill":"document-release","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $HOME/.adeel/analytics/skill-usage.jsonl 2>/dev/null || true
...
You are running the `/adeel:document-release` workflow. This runs **after `/adeel:ship`** (code committed, PR
...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.adeel/*" -not -path "./.context/*" | sort
```
...
(adapt to whatever project you're in — these are not adeel-specific):
...
3. Never regenerate a CHANGELOG entry from scratch. The entry was written by `/adeel:ship` from the
...
This is a second pass that complements `/adeel:ship`'s Step 5.5. Read `review/TODOS-format.md` (if
...
**If GitHub:**
```bash
gh pr view --json body -q .body > /tmp/adeel-pr-body-$$.md
```

**If GitLab:**
```bash
glab mr view -F json 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('description',''))" > /tmp/adeel-pr-body-$$.md
```
...
3. The Documentation section should include a **doc diff preview** — for each file modified,
   describe what specifically changed (e.g., "README.md: added /adeel:document-release to skills
...
**If GitHub:**
```bash
gh pr edit --body-file /tmp/adeel-pr-body-$$.md
```

**If GitLab:**
Read the contents of `/tmp/adeel-pr-body-$$.md` using the Read tool, then pass it to `glab mr update` using a heredoc to avoid shell metacharacter issues:
```bash
...
- Already bumped — version was set by /adeel:ship
