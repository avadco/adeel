...
description: |
  ...
  screenshots. For plan-mode design review (before implementation), use /adeel:plan-design-review.
...
echo '{"skill":"design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $HOME/.adeel/analytics/skill-usage.jsonl 2>/dev/null || true
...
"Your working tree has uncommitted changes. /adeel:design-review needs a clean tree so each design fix gets its own atomic commit."
...
**Find the adeel designer (optional — enables target mockup generation):**
...
REPORT_DIR=$HOME/.adeel/projects/$SLUG/designs/design-audit-$(date +%Y%m%d)
...
$B url
```
If URL contains `/login`, `/signin`, `/auth`, or `/sso`: the site requires authentication. AskUserQuestion: "This site requires authentication. Want to import cookies from your browser? Run `/adeel:setup-browser-cookies` first if needed."
...
**Local:** `.adeel/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:**
```bash
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/adeel-slug 2>/dev/null)" && mkdir -p $HOME/.adeel/projects/$SLUG
```
Write to: `$HOME/.adeel/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
```
$HOME/.adeel/projects/$SLUG/designs/design-audit-{YYYYMMDD}/
...
Use the same scorecard format as /adeel:plan-design-review (shown above). Fill in from both outputs.
...
If the adeel designer is available and the finding involves visual layout, hierarchy, or spacing (not just a CSS value fix like wrong color or font-size), generate a target mockup showing what the corrected version should look like:
...
For CSS-only fixes: skip entirely. CSS regressions are caught by re-running /adeel:design-review.

If the fix involved JS behavior: follow the same procedure as /adeel:qa Phase 8e.5 (study existing
...
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/adeel-slug 2>/dev/null)" && mkdir -p $HOME/.adeel/projects/$SLUG
```
Write a one-line summary to `$HOME/.adeel/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
...
2. **Fixed findings that were in TODOS.md** → annotate with "Fixed by /adeel:design-review on {branch}, {date}"
