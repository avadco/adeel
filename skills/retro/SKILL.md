...
echo '{"skill":"retro","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $HOME/.adeel/analytics/skill-usage.jsonl 2>/dev/null || true
...
# /adeel:retro — Weekly Engineering Retrospective

Generates a comprehensive engineering retrospective analyzing commit history, work patterns, and code quality metrics. Team-aware: identifies the user running the command, then analyzes every contributor with per-person praise and growth opportunities. Designed for a senior IC/CTO-level builder using Claude Code as a force multiplier.

## User-invocable
When the user types `/adeel:retro`, run this skill.

## Arguments
- `/adeel:retro` — default: last 7 days
- `/adeel:retro 24h` — last 24 hours
- `/adeel:retro 14d` — last 14 days
- `/adeel:retro 30d` — last 30 days
- `/adeel:retro compare` — compare current window vs prior same-length window
- `/adeel:retro compare 14d` — compare with explicit window
- `/adeel:retro global` — cross-project retro across all AI coding tools (7d default)
- `/adeel:retro global 14d` — cross-project retro with explicit window
...
```
Usage: /adeel:retro [window | compare | global]
  /adeel:retro              — last 7 days (default)
  /adeel:retro 24h          — last 24 hours
  /adeel:retro 14d          — last 14 days
  /adeel:retro 30d          — last 30 days
  /adeel:retro compare      — compare this period vs prior period
  /adeel:retro compare 14d  — compare with explicit window
  /adeel:retro global       — cross-project retro across all AI tools (7d default)
  /adeel:retro global 14d   — cross-project retro with explicit window
```
...
cat $HOME/.adeel/greptile-history.md 2>/dev/null || true
...
# 12. adeel skill usage telemetry (if available)
cat $HOME/.adeel/analytics/skill-usage.jsonl 2>/dev/null || true
...
**Greptile signal (if history exists):** Read `$HOME/.adeel/greptile-history.md` (fetched in Step 1, command 8). Filter entries within the retro time window by date. Count entries by type: `fix`, `fp`, `already-fixed`. Compute signal ratio: `(fix + already-fixed) / (fix + already-fixed + fp)`. If no entries exist in the window or the file doesn't exist, skip the Greptile metric row. Skip unparseable lines silently.
...
**Skill Usage (if analytics exist):** Read `$HOME/.adeel/analytics/skill-usage.jsonl` if it exists. Filter entries within the retro time window by `ts` field. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name. Present as:

```
| Skill Usage | /adeel:ship(12) /adeel:qa(8) /adeel:review(5) · 3 safety hook fires |
```

If the JSONL file doesn't exist or has no entries in the window, skip the Skill Usage row.

**Eureka Moments (if logged):** Read `$HOME/.adeel/analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
```
  EUREKA /adeel:office-hours (branch: user/auth-rethink): "Session tokens don't need server storage — browser crypto API makes client-side JWT validation viable"
  EUREKA /adeel:plan-eng-review (branch: user/cache-layer): "Redis isn't needed here — Bun's built-in LRU cache handles this workload"
```
...
**Note:** Only include the `greptile` field if `$HOME/.adeel/greptile-history.md` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (command 10 returns > 0). If any has no data, omit the field entirely.
...
### Plan Completion
Check review JSONL logs for plan completion data from /adeel:ship runs this period:

```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/adeel-slug 2>/dev/null)"
cat $HOME/.adeel/projects/$SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
When the user runs `/adeel:retro global` (or `/adeel:retro global 14d`), follow this flow instead of the repo-scoped Steps 1-14. This mode works from any directory — it does NOT require being inside a git repo.
...
```bash
DISCOVER_BIN=""
[ -x ${CLAUDE_PLUGIN_ROOT}/bin/adeel-global-discover ] && DISCOVER_BIN=${CLAUDE_PLUGIN_ROOT}/bin/adeel-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x ${CLAUDE_PLUGIN_ROOT}/bin/adeel-global-discover ] && DISCOVER_BIN=${CLAUDE_PLUGIN_ROOT}/bin/adeel-global-discover
[ -z "$DISCOVER_BIN" ] && which adeel-global-discover >/dev/null 2>&1 && DISCOVER_BIN=$(which adeel-global-discover)
[ -z "$DISCOVER_BIN" ] && [ -f bin/adeel-global-discover.ts ] && DISCOVER_BIN="bun run bin/adeel-global-discover.ts"
echo "DISCOVER_BIN: $DISCOVER_BIN"
```

If no binary is found, tell the user: "Discovery script not found. Run `bun run build` in the adeel directory to compile it." and stop.

Run the discovery:
```bash
$DISCOVER_BIN --since "<window>" --format json 2>/tmp/adeel-discover-stderr
```

Read the stderr output from `/tmp/adeel-discover-stderr` for diagnostic info. Parse the JSON output from stdout.

If `total_sessions` is 0, say: "No AI coding sessions found in the last <window>. Try a longer window: `/adeel:retro global 30d`" and stop.
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  E.g., "Built /adeel:retro global — cross-project retrospective with AI session discovery"
  not "feat: adeel-global-discover" + "feat: /adeel:retro global template".
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t $HOME/.adeel/retros/global-*.json 2>/dev/null | head -5
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls $HOME/.adeel/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `$HOME/.adeel/retros/global-${today}-${next}.json`:
...
```json
{
  ...
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | CC: 48, Codex: 8, Gemini: 3 | Focus: adeel (58%) | Streak: 52d"
}
```
...
When the user runs `/adeel:retro compare` (or `/adeel:retro compare 14d`):
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `$HOME/.adeel/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
