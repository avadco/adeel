...
echo '{"skill":"canary","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $HOME/.adeel/analytics/skill-usage.jsonl 2>/dev/null || true
...
# /adeel:canary — Post-Deploy Visual Monitor
...
## User-invocable
When the user types `/adeel:canary`, run this skill.

## Arguments
- `/adeel:canary <url>` — monitor a URL for 10 minutes after deploy
- `/adeel:canary <url> --duration 5m` — custom monitoring duration (1m to 30m)
- `/adeel:canary <url> --baseline` — capture baseline screenshots (run BEFORE deploying)
- `/adeel:canary <url> --pages /,/dashboard,/settings` — specify pages to monitor
- `/adeel:canary <url> --quick` — single-pass health check (no continuous monitoring)
...
```bash
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/adeel-slug 2>/dev/null || echo "SLUG=unknown")"
mkdir -p .adeel/canary-reports
mkdir -p .adeel/canary-reports/baselines
mkdir -p .adeel/canary-reports/screenshots
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".adeel/canary-reports/baselines/<page-name>.png"
$B console --errors
$B perf
$B text
```
...
Save the baseline manifest to `.adeel/canary-reports/baseline.json`:
...
Then STOP and tell the user: "Baseline captured. Deploy your changes, then run `/adeel:canary <url>` to monitor."
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".adeel/canary-reports/screenshots/pre-<page-name>.png"
$B console --errors
$B perf
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".adeel/canary-reports/screenshots/<page-name>-<check-number>.png"
$B console --errors
$B perf
```
...
Screenshots:   .adeel/canary-reports/screenshots/

VERDICT: [DEPLOY IS HEALTHY / DEPLOY HAS ISSUES — details above]
```

Save report to `.adeel/canary-reports/{date}-canary.md` and `.adeel/canary-reports/{date}-canary.json`.
