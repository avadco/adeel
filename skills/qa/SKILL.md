...
description: |
  ...
  fix evidence, and a ship-readiness summary. For report-only mode, use /adeel:qa-only.
allowed-tools:
  - Bash
  - Read
  - Write
...
echo '{"skill":"qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $HOME/.adeel/analytics/skill-usage.jsonl 2>/dev/null || true
...
| Mode | full | `--regression .adeel/qa-reports/baseline.json` |
| Output dir | `.adeel/qa-reports/` | `Output to /tmp/qa` |
...
"Your working tree has uncommitted changes. /adeel:qa needs a clean tree so each bug fix gets its own atomic commit."
...
```bash
mkdir -p .adeel/qa-reports/screenshots
```
...
1. **Project-scoped test plans:** Check `$HOME/.adeel/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   setopt +o nomatch 2>/dev/null || true  # zsh compat
   eval "$(${CLAUDE_PLUGIN_ROOT}/bin/adeel-slug 2>/dev/null)"
   ls -t $HOME/.adeel/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
2. **Conversation context:** Check if a prior `/adeel:plan-eng-review` or `/adeel:plan-ceo-review` produced test plan output in this conversation
...
This is the **primary mode** for developers verifying their work. When the user says `/adeel:qa` without a URL and the repo is on a feature branch, automatically:
...
2. **Identify affected pages/routes** from the changed files:
   ...
   **If no obvious pages/routes are identified from the diff:** Do not skip browser testing. The user invoked /adeel:qa because they want browser-based verification. Fall back to Quick mode — navigate to the homepage, follow the top 5 navigation targets, check console for errors, and test any interactive elements found. Backend, config, and infrastructure changes affect app behavior — always verify the app still works.
...
- Include full attribution comment:
  ...
  // Found by /adeel:qa on {YYYY-MM-DD}
  // Report: .adeel/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.adeel/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:** Write test outcome artifact for cross-session context:
```bash
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/adeel-slug 2>/dev/null)" && mkdir -p $HOME/.adeel/projects/$SLUG
```
Write to `$HOME/.adeel/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
...
2. **Fixed bugs that were in TODOS.md** → annotate with "Fixed by /adeel:qa on {branch}, {date}"
