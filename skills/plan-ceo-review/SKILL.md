...
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $HOME/.adeel/analytics/skill-usage.jsonl 2>/dev/null || true
...
DESIGN=$(ls -t $HOME/.adeel/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t $HOME/.adeel/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
[ -n "$DESIGN" ] && echo "Design doc found: $DESIGN" || echo "No design doc found"
```
If a design doc exists (from `/adeel:office-hours`), read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design.

**Handoff note check** (reuses $SLUG and $BRANCH from the design doc check above):
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t $HOME/.adeel/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
from a prior CEO review session that paused so the user could run `/adeel:office-hours`. Use it
...
> "No design doc found for this branch. `/adeel:office-hours` produces a structured problem
> statement, premise challenge, and explored alternatives — it gives this review much
> sharper input to work with. Takes about 10 minutes. The design doc is per-feature,
> not per-product — it captures the thinking behind this specific change."

Options:
- A) Run /adeel:office-hours now (we'll pick up the review right after)
- B) Skip — proceed with standard review

If they skip: "No worries — standard review. If you ever want sharper input, try
/adeel:office-hours first next time." Then proceed normally. Do not re-offer later in the session.

If they choose A:

Say: "Running /adeel:office-hours inline. Once the design doc is ready, I'll pick up
...
"Could not load /adeel:office-hours — proceeding with standard review."

After /adeel:office-hours completes, re-run the design doc check:
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
SLUG=$(${CLAUDE_PLUGIN_ROOT}/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
...
sure," or is clearly exploring rather than reviewing — offer `/adeel:office-hours`:

> "It sounds like you're still figuring out what to build — that's totally fine, but
> that's what /adeel:office-hours is designed for. Want to run /adeel:office-hours right now?
> We'll pick up right where we left off."

Options: A) Yes, run /adeel:office-hours now. B) No, keep going.
...
```bash
eval "$(${CLAUDE_PLUGIN_ROOT}/bin/adeel-slug 2>/dev/null)" && mkdir -p $HOME/.adeel/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p $HOME/.adeel/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv $HOME/.adeel/projects/$SLUG/ceo-plans/{old-plan}.md $HOME/.adeel/projects/$SLUG/ceo-plans/archive/
```

Write to `$HOME/.adeel/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
Generated by /adeel:plan-ceo-review on {date}
...
```bash
mkdir -p $HOME/.adeel/analytics
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> $HOME/.adeel/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```
NOTE: These represent human-team implementation hours. With CC + adeel,
...
The CEO calling in the designer. Not a pixel-level audit — that's /adeel:plan-design-review and /adeel:design-review. This is ensuring the plan has design intentionality.
...
If this plan has significant UI scope, recommend: "Consider running /adeel:plan-design-review for a deep design review of this plan before implementation."
**STOP.** AskUserQuestion once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.
...
**Persist the result:**
```bash
${CLAUDE_PLUGIN_ROOT}/bin/adeel-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
After implementation, run `/adeel:design-review` on the live site to catch visual issues that can only be evaluated with rendered output.
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+adeel: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
rm -f $HOME/.adeel/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
`$HOME/.adeel/` (user config directory, not project files). The skill preamble
already writes to `$HOME/.adeel/sessions/` and `$HOME/.adeel/analytics/` — this is
the same pattern. The review dashboard depends on this data. Skipping this
command breaks the review readiness dashboard in /adeel:ship.

```bash
${CLAUDE_PLUGIN_ROOT}/bin/adeel-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```
...
| Review | Trigger | Why | Runs | Status | Findings |
|--------|---------|-----|------|--------|----------|
| CEO Review | \`/adeel:plan-ceo-review\` | Scope & strategy | {runs} | {status} | {findings} |
| Codex Review | \`/adeel:codex review\` | Independent 2nd opinion | {runs} | {status} | {findings} |
| Eng Review | \`/adeel:plan-eng-review\` | Architecture & tests (required) | {runs} | {status} | {findings} |
| Design Review | \`/adeel:plan-design-review\` | UI/UX gaps | {runs} | {status} | {findings} |
\`\`\`
...
- Search the plan file for a \`## ADEEL REVIEW REPORT\` section **anywhere** in the file
  (not just at the end — content may have been added after it).
- If found, **replace it** entirely using the Edit tool. Match from \`## ADEEL REVIEW REPORT\`
...
**Recommend /adeel:plan-eng-review if eng review is not skipped globally** — check the dashboard output for `skip_eng_review`. If it is `true`, eng review is opted out — do not recommend it. Otherwise, eng review is the required shipping gate. If this CEO review expanded scope, changed architectural direction, or accepted scope expansions, emphasize that a fresh eng review is needed. If an eng review already exists in the dashboard but the commit hash shows it predates this CEO review, note that it may be stale and should be re-run.

**Recommend /adeel:plan-design-review if UI scope was detected** — specifically if Section 11 (Design & UX Review) was NOT skipped, or if accepted scope expansions included UI-facing features. If an existing design review is stale (commit hash drift), note that. In SCOPE REDUCTION mode, skip this recommendation — design review is unlikely relevant for scope cuts.

**If both are needed, recommend eng review first** (required gate), then design review.

Use AskUserQuestion to present the next step. Include only applicable options:
- **A)** Run /adeel:plan-eng-review next (required gate)
- **B)** Run /adeel:plan-design-review next (only if UI scope detected)
...
- **B)** Keep in `$HOME/.adeel/projects/` only (local, personal reference)
- **C)** Skip
