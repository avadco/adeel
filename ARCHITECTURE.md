...
This document explains **why** adeel is built the way it is. For setup and commands, see CLAUDE.md. For contributing, see CONTRIBUTING.md.

## The core idea

adeel gives Claude Code a persistent browser and a set of opinionated workflow skills. The browser is the hard part — everything else is Markdown.

The key insight: an AI agent interacting with a browser needs **sub-second latency** and **persistent state**. If every command cold-starts a browser, you're waiting 3-5 seconds per tool call. If the browser dies between commands, you lose cookies, tabs, and login sessions. So adeel runs a long-lived Chromium daemon that the CLI talks to over localhost HTTP.
...
1. **Compiled binaries.** `bun build --compile` produces a single ~58MB executable. No `node_modules` at runtime, no `npx`, no PATH configuration. The binary just runs. This matters because adeel installs into `~/.claude/skills/` where users don't expect to manage a Node.js project.
...
The server writes `.adeel/browse.json` (atomic write via tmp + rename, mode 0o600):
...
Cookies are the most sensitive data adeel handles. The design:

1. **Keychain access requires user approval.** First cookie import per browser triggers a macOS Keychain dialog. The user must click "Allow" or "Always Allow." adeel never silently accesses credentials.
...
3. **Database is read-only.** adeel copies the Chromium cookie DB to a temp file (to avoid SQLite lock conflicts with the running browser) and opens it read-only. It never modifies your real browser's cookie database.
...
```
Browser events → CircularBuffer (in-memory) → Async flush to .adeel/*.log
```
...
| `{{QA_METHODOLOGY}}` | `gen-skill-docs.ts` | Shared QA methodology block for /adeel:qa and /adeel:qa-only |
| `{{DESIGN_METHODOLOGY}}` | `gen-skill-docs.ts` | Shared design audit methodology for /adeel:plan-design-review and /adeel:design-review |
| `{{REVIEW_DASHBOARD}}` | `gen-skill-docs.ts` | Review Readiness Dashboard for /adeel:ship pre-flight |
| `{{TEST_BOOTSTRAP}}` | `gen-skill-docs.ts` | Test framework detection, bootstrap, CI/CD setup for /adeel:qa, /adeel:ship, /adeel:design-review |
| `{{CODEX_PLAN_REVIEW}}` | `gen-skill-docs.ts` | Optional cross-model plan review (Codex or Claude subagent fallback) for /adeel:plan-ceo-review and /adeel:plan-eng-review |
| `{{DESIGN_SETUP}}` | `resolvers/design.ts` | Discovery pattern for `$D` design binary, mirrors `{{BROWSE_SETUP}}` |
| `{{DESIGN_SHOTGUN_LOOP}}` | `resolvers/design.ts` | Shared comparison board feedback loop for /adeel:design-shotgun, /adeel:plan-design-review, /adeel:design-consultation |
...
1. **Update check** — calls `adeel-update-check`, reports if an upgrade is available.
2. **Session tracking** — touches `$HOME/.adeel/sessions/$PPID` and counts active sessions (files modified in the last 2 hours). When 3+ sessions are running, all skills enter "ELI16 mode" — every question re-grounds the user on context because they're juggling windows.
3. **Contributor mode** — reads `adeel_contributor` from config. When true, the agent files casual field reports to `$HOME/.adeel/contributor-logs/` when adeel itself misbehaves.
...
1. **Claude reads SKILL.md at skill load time.** There's no build step when a user invokes `/adeel:browse`. The file must already exist and be correct.
...
```bash
jq '.tests[] | select(.exit_reason == "timeout") | .last_tool_call' $HOME/.adeel-dev/evals/_partial-e2e.json
```
...
`eval:compare` diffs two eval runs. `eval:summary` aggregates stats across all runs in `$HOME/.adeel-dev/evals/`.
