# Contributing to adeel

Thanks for wanting to make adeel better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

adeel skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `${CLAUDE_PLUGIN_ROOT}/` (your global install). But when you're developing adeel itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.

That's what dev mode does. It symlinks your repo into the local `.claude/skills/` directory so Claude Code reads skills straight from your checkout.

```bash
git clone <repo> && cd adeel
bun install                    # install dependencies
bin/dev-setup                  # activate dev mode
```

Now edit any `SKILL.md`, invoke it in Claude Code (e.g. `/adeel:review`), and see your changes live. When you're done developing:
...
Contributor mode turns adeel into a self-improving tool. Enable it and Claude Code
will periodically reflect on its adeel experience — rating it 0-10 at the end of
each major workflow step. When something isn't a 10, it thinks about why and files
a report to `$HOME/.adeel/contributor-logs/` with what happened, repro steps, and what
would make it better.

```bash
${CLAUDE_PLUGIN_ROOT}/bin/adeel-config set adeel_contributor true
```

The logs are for **you**. When something bugs you enough to fix, the report is
already written. Fork adeel, symlink your fork into the project where you hit
the issue, fix it, and open a PR.

### The contributor workflow

1. **Use adeel normally** — contributor mode reflects and logs issues automatically
2. **Check your logs:** `ls $HOME/.adeel/contributor-logs/`
3. **Fork and clone adeel** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where adeel annoyed you)
   ln -sfn /path/to/your/adeel-fork .claude/skills/adeel
   cd .claude/skills/adeel && bun install && bun run build && ./setup
   ```
   Setup creates the per-skill symlinks (`qa -> adeel/qa`, etc.) and asks your
   prefix preference. Pass `--no-prefix` to skip the prompt and use short names.
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using adeel** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix adeel while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ adeel sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all skills.

## Working on adeel inside the adeel repo

When you're editing adeel skills and want to test them by actually using adeel
...
```
adeel/                          <- your working tree
├── .claude/skills/              <- created by dev-setup (gitignored)
│   ├── adeel -> ../../         <- symlink back to repo root
│   ├── review -> adeel/review  <- short names (default)
│   ├── ship -> adeel/ship      <- or adeel-review, adeel-ship if --prefix
│   └── ...                      <- one symlink per skill
├── review/
│   └── SKILL.md                 <- edit this, test with /adeel:review
├── ship/
│   └── SKILL.md
├── browse/
...
Skill symlink names depend on your prefix setting (`$HOME/.adeel/config.yaml`).
Short names (`/adeel:review`, `/adeel:ship`) are the default. Run `./setup --prefix` if you
prefer namespaced names (`/adeel-review`, `/adeel-ship`).
...
When E2E tests run, they produce machine-readable artifacts in `$HOME/.adeel-dev/`:
...
Artifacts are never cleaned up — they accumulate in `$HOME/.adeel-dev/` for post-mortem debugging and trend analysis.
...
adeel generates SKILL.md files for two hosts: **Claude** (`.claude/skills/`) and **Codex** (`.agents/skills/`). Every template change needs to be generated for both.
...
| Aspect | Claude | Codex |
|--------|--------|-------|
| Output directory | `{skill}/SKILL.md` | `.agents/skills/adeel-{skill}/SKILL.md` (generated at setup, gitignored) |
| Frontmatter | Full (name, description, allowed-tools, hooks, version) | Minimal (name + description only) |
| Paths | `~/.claude/skills/adeel` | `$ADEEL_ROOT` (`.agents/skills/adeel` in a repo, otherwise `~/.codex/skills/adeel`) |
| Hook skills | `hooks:` frontmatter (enforced by Claude) | Inline safety advisory prose (advisory only) |
| `/adeel:codex` skill | Included (Claude wraps codex exec) | Excluded (self-referential) |
...
- **TODOS.md is the unified backlog.** Organized by skill/component with P0-P4 priorities. `/adeel:ship` auto-detects completed items. All planning/review/retro skills read it for context.
- **Browse source changes need a rebuild.** If you touch `browse/src/*.ts`, run `bun run build`.
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.claude/skills/adeel`. `bin/dev-teardown` restores the global one.
...
**This is the recommended way to develop adeel.** Symlink your adeel checkout
...
```bash
# In your core project (not the adeel repo)
ln -sfn /path/to/your/adeel-checkout .claude/skills/adeel
```

### Step 2: Run setup to create per-skill symlinks

The `adeel` symlink alone isn't enough. Claude Code discovers skills through
individual symlinks (`qa -> adeel/qa`, `ship -> adeel/ship`, etc.), not through
the `adeel/` directory itself. Run `./setup` to create them:

```bash
cd .claude/skills/adeel && bun install && bun run build && ./setup
```

Setup will ask whether you want short names (`/adeel:qa`) or namespaced (`/adeel-qa`).
Your choice is saved to `$HOME/.adeel/config.yaml` and remembered for future runs.
To skip the prompt, pass `--no-prefix` (short names) or `--prefix` (namespaced).

### Step 3: Develop

Edit a template, run `bun run gen:skill-docs`, and the next `/adeel:review` or `/adeel:qa`
call picks it up immediately. No restart needed.

### Going back to the stable global install

Remove the project-local symlink. Claude Code falls back to `${CLAUDE_PLUGIN_ROOT}/`:

```bash
rm .claude/skills/adeel
```

The per-skill symlinks (`qa`, `ship`, etc.) still point to `adeel/...`, so they'll
resolve to the global install automatically.

### Switching prefix mode

If you vendored adeel with one prefix setting and want to switch:

```bash
cd .claude/skills/adeel && ./setup --no-prefix   # switch to /adeel:qa, /adeel:ship
cd .claude/skills/adeel && ./setup --prefix       # switch to /adeel-qa, /adeel-ship
```
